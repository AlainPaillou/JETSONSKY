...
#        Images/videos acquisition & treatment software for ZWO ASI178MC/MC, ASI290MM/MC, ASI224MC         #
#                                       ASI485MC & ASI 585MC camera                                        #
#                                                                                                          #
...
# Choose the right repertories for video & image  PC DD/SSD 
# Choose the right repertories for ZWO libray
...
# Titre programme
titre = "JetsonSky V14.03 Windows release - CUDA / OPENCV - Copyright Alain PAILLOU 2018-2022"
...
def init_camera() :
    ...
    if cameras_found[0] == "ZWO ASI290MC" or cameras_found[0] == "ZWO ASI290MM" :
        res_x_max = 1936
        res_y_max = 1096
        res_cam_x = 1936
        res_cam_y = 1096
    ...
    if flag_filtrage_ON == True :
        ...
        if flag_noir_blanc == 1 or flag_colour_camera == False :
            ...
            if flag_contrast_CLAHE ==1 :
                if flag_type_clahe == 1 :
                    clahe = cv2.createCLAHE(clipLimit=val_contrast_CLAHE, tileGridSize=(8,8))
                    imag_brute = clahe.apply(imag_brute)
                if flag_type_clahe == 2 :
                    clahe = cv2.createCLAHE(clipLimit=val_contrast_CLAHE, tileGridSize=(8,8))
                    imag_brute_blur = cv2.GaussianBlur(imag_brute,(7,7),cv2.BORDER_DEFAULT,)
                    res_bb = clahe.apply(imag_brute_blur)
                    imag_brute = cv2.addWeighted(imag_brute, 0.65, res_bb, 0.35, 0)
            ...
            if (val_FS < 2 or flag_hold_picture == 1) :
                image_traitee = imag_brute
        ...
        if flag_noir_blanc == 0 and flag_colour_camera == True:
            ...
            if flag_contrast_CLAHE ==1 :
                ...
                if flag_type_clahe == 2 :
                    clahe = cv2.createCLAHE(clipLimit=val_contrast_CLAHE, tileGridSize=(8,8))
                    image_brut_CV=cv2.merge((res_b1,res_g1,res_r1))
                    image_brut_blur = cv2.GaussianBlur(image_brut_CV,(7,7),cv2.BORDER_DEFAULT,)
                    res_bb, res_gb, res_rb = cv2.split(image_brut_blur)
                    res_bb = clahe.apply(res_bb)
                    res_gb = clahe.apply(res_gb)
                    res_rb = clahe.apply(res_rb)
                    image_brut_blur = cv2.merge((res_bb,res_gb,res_rb))
                    image_brut_CV = cv2.addWeighted(image_brut_CV, 0.65, image_brut_blur, 0.35, 0)
                    res_b1, res_g1, res_r1 = cv2.split(image_brut_CV)
...
echelle60.place(anchor="w", x=1530,y=570)

# Choix Parametre Atenuation Gradient Removal
labelParam61 = Label (cadre, text = "At")
labelParam61.place(anchor="e", x=1720,y=570)
...
echelle61.place(anchor="w", x=1720,y=570)
...
CBIN.place(anchor="w",x=1450, y=635)
...
CBHE2.place(anchor="w",x=1520, y=635)
...
echelle16.place(anchor="w", x=1580,y=635)
...
echelle5.place(anchor="w", x=1580,y=680)

labelParam6 = Label (cadre, text = "Max") # choix valeur histogramme strech maximum
labelParam6.place(anchor="w", x=1700,y=680)
...
echelle6.place(anchor="w", x=1720,y=680)
...
echelle12.place(anchor="w", x=1580,y=725)

labelParam13 = Label (cadre, text = "Dec") # choix valeur histogramme Signoide param 2
labelParam13.place(anchor="w", x=1700,y=725)
...
echelle13.place(anchor="w", x=1720,y=725)
...
CBCC.place(anchor="w",x=1450, y=770)

labelParam9 = Label (cadre, text = "Clip") # choix valeur contrate CLAHE
labelParam9.place(anchor="w", x=1555,y=770)
...
echelle9.place(anchor="w", x=1580,y=770)
...
Button6.place(anchor="w", x=1460,y=830)
