...
# Choose your system Jetson system or PC Windows system
...
Video_Test ='D:\Soft\JetsonSky\Videos\Your_Video.avi'

# Choose the directories to save video treatments and images captures
image_path = 'D:\Soft\JetsonSky\Images'
video_path= 'D:\Soft\JetsonSky\Videos'


######################################################################
# Don't change thoses settings - They can be changes from the software

titre = "JetsonTreatment PC version V8.00 - CUDA / OPENCV - Copyright Alain PAILLOU 2018-2022"
flag_HQ = 0 
exp_min=100 #µs
exp_max=10000 #µs
exp_delta=100 #µs
...
sat_old_x = np.zeros(1000,dtype=int)
sat_old_y = np.zeros(1000,dtype=int)
sat_id  = np.zeros(1000,dtype=int)

correspondance = np.zeros(1000,dtype=int)
vitesse = np.zeros(1000,dtype=int)
...
mod = SourceModule("""
__global__ void Mono_ampsoft_GPU(unsigned char *dest_r, unsigned char *img_r, long int width, long int height, float val_ampl, float *Corr_GS)
{
...
unsigned char *img_r, unsigned char *img_g, unsigned char *img_b, long int width, long int height, float val_ampl, float *Corr_GS)
{
  ...
  if (i < height && j < width) {
//      vr = (int)(int(img_r[index] * 0.299 + img_g[index] * 0.587 + img_b[index] * 0.114));
      vr = (int)(int(img_r[index] * 0.2126 + img_g[index] * 0.7152 + img_b[index] * 0.0722));
      dest_r[index] = (int)(min(max(int(img_r[index] * (1.0 + (val_ampl-1.0)*Corr_GS[vr])), 0), 255));
      dest_g[index] = (int)(min(max(int(img_g[index] * (1.0 + (val_ampl-1.0)*Corr_GS[vr])), 0), 255));
      dest_b[index] = (int)(min(max(int(img_b[index] * (1.0 + (val_ampl-1.0)*Corr_GS[vr])), 0), 255));
      img_r[index] = dest_r[index];
...
//      mul_factor = __expf(-0.5*__powf((grey_gpu[index]*0.03921568-5-val_Mu)/val_Ro,2));
      index_grey = grey_gpu[index];
      mul_factor = Corr_GS[index_grey];
      dest_r[index] = (int)(min(max(int(img_r[index] + delta*mul_factor*val_ampl*2), 0), 255));
      dest_g[index] = (int)(min(max(int(img_g[index] + delta*mul_factor*val_ampl*2), 0), 255));
      dest_b[index] = (int)(min(max(int(img_b[index] + delta*mul_factor*val_ampl*2), 0), 255));
  }
}
...
mod = SourceModule("""
__global__ void Smooth_Color_high(unsigned char *dest_r, unsigned char *dest_g, unsigned char *dest_b, unsigned char *img_r, unsigned char *img_g,
unsigned char *img_b, long int width, long int height, int flag_2DConv)
{
...
Smooth_Color_high = mod.get_function("Smooth_Color_high")


mod = SourceModule("""
__global__ void GaussianBlur_Color(unsigned char *dest_r, unsigned char *dest_g, unsigned char *dest_b, unsigned char *img_r, unsigned char *img_g,
unsigned char *img_b, long int width, long int height)
{
...
GaussianBlur_Color = mod.get_function("GaussianBlur_Color")
...
def refresh() :
    global val_nb_capt_video,flag_cap_video,image_brut_Base,camera,video,traitement, img_cam,cadre_image,rawCapture,image_affichee,image_brut_CV,flag_image_disponible,\
           thread_1,flag_acquisition_en_cours,flag_autorise_acquisition,flag_premier_demarrage,flag_BIN2,image_traitee,val_SAT,imggrey1,imggrey2,\
          imggrey1,imggrey2,flag_FDIF,FDIF,flag_TRKSAT,calque_stars,calque_satellites,flag_nouvelle_resolution,nb_sat,sat_x,sat_y,sat_old_x,sat_old_y,calque_met,\
          correspondance,vitesse
    ...
    if (video.isOpened()):
        ...
        if ret == True :
            ...
            if flag_nouvelle_resolution == True :
                calque_stars = image_brut_Base
                cr,cg,cb = cv2.split(calque_stars)
                cr = cr * 0
                cg = cg * 0
                cb = cb * 0
                calque_stars = cv2.merge((cb,cg,cr))
                calque_satellites = cv2.merge((cb,cg,cr))
                calque_met = cv2.merge((cb,cg,cr))
        ...
        if flag_TRKSAT == 1 or flag_FDIF == 1 or flag_TRKMET == 1:
            stars_satellites_met_tracking()
            if flag_TRKSAT == 1 :
                ...
                for i in range(nb_sat+1):
                    if correspondance[i] >=0:
                        ...
                        cv2.circle(image_traitee, centercircle, 5, (0,255,0), 1, cv2.LINE_AA)
            ...
            image_traitee = cv2.addWeighted(image_traitee, 1, calque_met, 1, 0)     
        ...
        if flag_HST == 1 and flag_noir_blanc == 0 :
        ...
        if flag_HST == 1 and flag_noir_blanc == 1 :
            r = cadre_image.im
            hst_r = r.histogram()
            histo = PIL.ImageDraw.Draw(cadre_image.im)
            for x in range(1,256) :
            ...
            histo.line(((256*3,cam_displ_y),(256*3,cam_displ_y-256*3)),fill="red",width=3)
            histo.line(((1,cam_displ_y-256*3),(256*3,cam_displ_y-256*3)),fill="red",width=3)
        if flag_TRSF == 1 and flag_noir_blanc == 0 :
        ...
        if flag_TRSF == 1 and flag_noir_blanc == 1 :
            transform = PIL.ImageDraw.Draw(cadre_image.im)
            for x in range(2,256) :
                transform.line((((x-1)*3,cam_displ_y-trsf_r[x-1]*3),(x*3,cam_displ_y-trsf_r[x]*3)),fill="white",width=2)
            transform.line(((256*3,cam_displ_y),(256*3,cam_displ_y-256*3)),fill="red",width=3)
            transform.line(((1,cam_displ_y-256*3),(256*3,cam_displ_y-256*3)),fill="red",width=3)
        if flag_TRGS == 1:
            transform = PIL.ImageDraw.Draw(cadre_image.im)
...
def stars_satellites_met_tracking ():
    global imggrey1,imggrey2,image_traitee,calque_satellites,flag_sat_exist,sat_x,sat_y,sat_s,sat_old_x,sat_old_y,sat_id,nb_sat,\
           flag_TRKSAT,flag_FDIF,calque_stars,flag_TRKMET,calque_met,correspondance,vitesse,labelInfo1

    correspondance = (correspondance * 0) - 1

    if flag_TRKSAT == 1:
        imggrey2 = cv2.cvtColor(image_traitee, cv2.COLOR_BGR2GRAY)
        try :
            diff = cv2.absdiff(imggrey2, imggrey1)
            diff[0:50,0:750] = 0
            imggrey1 = imggrey2
            ...
            params_sat.filterByColor = False    # Filter by color.    
            params_sat.blobColor = 0  # 0 for darkblobs - 255 for light blobs)
            params_sat.minDistBetweenBlobs = 10
            params_sat.filterByArea = True    # Filter by Area.
            params_sat.minArea = 5
            params_sat.maxArea = 150
            params_sat.minRepeatability = 1
            ...
            image_satellites=cv2.bilateralFilter(image_sat,7,65,65)
            keypoints_sat = detector_sat.detect(image_satellites)
            flag_sat = True
        except :
            flag_sat = False
            imggrey1 = imggrey2
            ...
            if flag_sat_exist == True :
                ...
                if nb_sat >= 0 :
                    ...
                    for kp_sat in keypoints_sat:
                        ...
                        sat_s[nb_sat] = int(kp_sat.size)
                    for i in range(nb_sat+1) :
                        dist_min = 100000
                        for j in range(nb_sat+1) :
    ...
    if flag_FDIF == 1 or flag_TRKMET == 1:
        cr,cg,cb = cv2.split(calque_stars)
        cr = cr * 0
        cg = cg * 0
        cb = cb * 0
        calque_stars = cv2.merge((cb,cg,cr))
        res_b,res_g,res_r = cv2.split(image_traitee)
        seuilb = np.percentile(res_b, 95)
        seuilg = np.percentile(res_g, 95)
        seuilr = np.percentile(res_r, 95)
        seuil_min_blob = max(seuilb,seuilg,seuilr) + 20
        ...
        params.blobColor = 0  # 0 for darkblobs - 255 for light blobs)
        ...
        image_etoiles=cv2.bilateralFilter(image_traitee,5,125,125)
        keypoints = detector.detect(image_etoiles)
        for kp in keypoints:
                ...
                if flag_FDIF == 1:
                    for i in range(nb_sat+1) :
                        ...
                        if int(kp.pt[0]) == sat_x[i] and int(kp.pt[1]) == sat_y[i] :
                            affiche = False
                    if affiche == True :
                        if int(kp.size) > 0 and int(kp.size) < 4:
                            colour = (0,255,255)
                        if int(kp.size) >= 4 and int(kp.size) < 8:
                            colour = (240,255,0)
                        if int(kp.size) >= 8 and int(kp.size) < 15:
                            colour = (255,127,0)
                        if int(kp.size) >= 15 and int(kp.size) < 35:
                            colour = (255,0,0)                   
                        cv2.circle(calque_stars, (int(kp.pt[0]), int(kp.pt[1])), int(kp.size), colour, 1, cv2.LINE_AA)
                if flag_TRKMET == 1:
                    if int(kp.size) >= 35:
                        cv2.circle(calque_met, (int(kp.pt[0]), int(kp.pt[1])), int(kp.size), (255,255,0), 2, cv2.LINE_AA)
    

def application_filtrage() :
    global image_traitee1,image_traitee2,image_traitee3,image_traitee4,image_traitee5,compteur_FS,Im1OK,Im2OK,Im3OK,\
           Im4OK,Im5OK,flag_hold_picture,image_brut_Base,img_brut_tmp,val_denoise,val_denoise_KNN,val_histo_min,val_histo_max, \
           flag_cap_pic,flag_traitement,val_contrast_CLAHE,flag_histogram_phitheta,image_traitee,flag_front,\
           val_heq2,val_SGR,val_NGB,val_AGR,flag_filtre_work,flag_AmpSoft,val_ampl,grad_vignet,compteur_FSDN,flag_lin_gauss,flag_SAT,val_SAT,\
           Im1fsdnOK,Im2fsdnOK,image_traiteefsdn1,image_traiteefsdn2,old_image,val_reds,val_greens,val_blues,trsf_r,trsf_g,trsf_b,flag_dyn_AADP,Corr_GS
    if flag_filtrage_ON == True :
        ...
        if flag_noir_blanc == 1 or flag_colour_camera == False :
            ...
            if flag_2DConv == 1 or ((flag_histogram_stretch == 1 or flag_histogram_equalize2 == 1 or flag_histogram_phitheta == 1) and flag_front == True) or\
               flag_AmpSoft == 1 or flag_denoise_Paillou == 1 or flag_denoise_soft == 1 or flag_denoise_KNN == 1 :
            ...
            if (flag_histogram_stretch == 1 or flag_histogram_equalize2 == 1 or flag_histogram_phitheta == 1) and flag_front == True :
            ...
            if flag_AmpSoft == 1 :
                if flag_lin_gauss == 1 or flag_lin_gauss == 2 :
                    ...
                    Mono_ampsoft_GPU(r_gpu, img_r_gpu,np.intc(width),np.intc(height), np.float32(val_ampl), correction, \
                         block=(nb_ThreadsX,nb_ThreadsY,1), grid=(nb_blocksX,nb_blocksY))
            ...
            if val_FS > 1 and Dev_system == "PC Windows" :
                ...
                if compteur_FS == 3 :
                    ...
                    Im3OK = True
                if compteur_FS == 4 :
                    image_traitee4 = image_brut_CV
                    Im4OK = True
                if compteur_FS == 5 :
                    image_traitee5 = image_brut_CV
                    Im5OK = True
                ...
                if val_FS == 2 and Im2OK == True :
                    nb_images = 2
                    res_r1 = image_traitee1
                    res_r2 = image_traitee2
                    ...
                    image_traitee=res_r1

                if val_FS == 3 and Im3OK == True :
                    nb_images = 3
                    res_r1 = image_traitee1
                    res_r2 = image_traitee2
                    res_r3 = image_traitee3
            ...
            # Image Negatif
            if ImageNeg == 1 and flag_front == False :
                image_traitee = cv2.bitwise_not(image_traitee) # Test fonction cv2.bitwise_not
                for x in range(1,256) :
                    trsf_r[x] = (int)(256-trsf_r[x])
                trsf_r[trsf_r > 255] = 255
            ...
            # Histo stretch CUDA
            # Histo Phi Theta CUDA           
            if (flag_histogram_stretch == 1 or flag_histogram_equalize2 == 1 or flag_histogram_phitheta == 1) and flag_front == False :
                
                r_gpu = drv.mem_alloc(image_brut_CV.size * image_traitee.dtype.itemsize)
                img_r_gpu = drv.mem_alloc(image_brut_CV.size * image_traitee.dtype.itemsize)
                drv.memcpy_htod(r_gpu, image_traitee)
                drv.memcpy_htod(img_r_gpu, image_traitee)
                res_r = np.empty_like(image_traitee)
        ...
        if flag_noir_blanc == 0 and flag_colour_camera == True:
            ...
            if flag_DEMO == 1 :
                image_initiale = image_brut_Base[0:height,0:width//2]
            nb_pixels = height * width
            nb_blocksX = (width // nb_ThreadsX) + 1
            nb_blocksY = (height // nb_ThreadsY) + 1
            ...
            if flag_denoise_stacking_Paillou == True :
                ...
                if compteur_FSDN < 3 :
                    if compteur_FSDN == 1 :
                        old_image = image_brut_Base
                        Im1fsdnOK = True
                    if compteur_FSDN == 2 :
                        Im2fsdnOK = True

                res_b1,res_g1,res_r1 = cv2.split(image_brut_Base)
                ...
                if Im2fsdnOK == True :
                    nb_images = 2
                    res_b1,res_g1,res_r1 = cv2.split(image_brut_Base)
                    res_b2,res_g2,res_r2 = cv2.split(old_image)
                    divise = 2.0
                ...
                image_brut_Base=cv2.merge((res_b1,res_g1,res_r1))
                old_image = image_brut_Base
            

            if flag_AmpSoft == 1 and flag_lin_gauss == 3 :
                imagegrey = cv2.cvtColor(image_brut_Base, cv2.COLOR_BGR2GRAY)
                imagegreyblur = imagegrey
                
                niveau_blur = 11
                imagegreyblur=cv2.GaussianBlur(imagegrey,(niveau_blur,niveau_blur),cv2.BORDER_DEFAULT)
                
                res_b,res_g,res_r = cv2.split(image_brut_Base)

                b_gpu = drv.mem_alloc(res_b.size * res_b.dtype.itemsize)
                img_b_gpu = drv.mem_alloc(res_b.size * res_b.dtype.itemsize)
            
                g_gpu = drv.mem_alloc(res_g.size * res_g.dtype.itemsize)
                img_g_gpu = drv.mem_alloc(res_g.size * res_g.dtype.itemsize)
            
                r_gpu = drv.mem_alloc(res_r.size * res_r.dtype.itemsize)
                img_r_gpu = drv.mem_alloc(res_r.size * res_r.dtype.itemsize)

                drv.memcpy_htod(img_b_gpu, res_b)  
                drv.memcpy_htod(img_g_gpu, res_g)
                drv.memcpy_htod(img_r_gpu, res_r)
                ...
                Colour_staramp_GPU(r_gpu, g_gpu, b_gpu, img_r_gpu, img_g_gpu, img_b_gpu, grey_gpu, grey_blur_gpu,np.int_(width), np.int_(height), \
                ...
                image_brut_Base=cv2.merge((res_b,res_g,res_r))
                image_traitee = image_brut_Base
            ...
            # Image negative
            if ImageNeg == 1 and flag_front == True :
                res_b = cv2.bitwise_not(res_b)
                res_g = cv2.bitwise_not(res_g)
                res_r = cv2.bitwise_not(res_r)
                ...
                trsf_r[trsf_r > 255] = 255
                trsf_g[trsf_g > 255] = 255
                trsf_b[trsf_b > 255] = 255


            # bilateral openCV
            if flag_bilateral == 1 :
                image_tempo = cv2.merge((res_b,res_g,res_r))
                image_tempo=cv2.bilateralFilter(image_tempo,5,125,125) # Application filtre bilateral
                res_b,res_g,res_r = cv2.split(image_tempo)
            ...
            if flag_denoise_soft == 1 :
                ...
                NLM2_Colour_GPU(r_gpu, g_gpu, b_gpu, img_r_gpu, img_g_gpu, img_b_gpu,np.int_(width),np.int_(height), np.float32(Noise), \
            ...
            if flag_GR == True :
                if grad_vignet == 1 :
                    seuilb = np.percentile(res_b, val_SGR)
                    seuilg = np.percentile(res_g, val_SGR)
                    seuilr = np.percentile(res_r, val_SGR)
                    th,img_b = cv2.threshold(res_b,seuilb,255,cv2.THRESH_TRUNC) 
                    th,img_g = cv2.threshold(res_g,seuilg,255,cv2.THRESH_TRUNC) 
                    th,img_r = cv2.threshold(res_r,seuilr,255,cv2.THRESH_TRUNC) 
                    niveau_blur = val_NGB*2 + 3
                    img_b=cv2.GaussianBlur(img_b,(niveau_blur,niveau_blur),cv2.BORDER_DEFAULT) 
                    ...
                    image_brut_CV = cv2.subtract(image_brut_CV,gradient)
                    res_b,res_g,res_r = cv2.split(image_brut_CV)
                    ...
                    th,fd_b = cv2.threshold(res_b,seuilb,255,cv2.THRESH_TRUNC) 
                    th,fd_g = cv2.threshold(res_g,seuilg,255,cv2.THRESH_TRUNC) 
                    th,fd_r = cv2.threshold(res_r,seuilr,255,cv2.THRESH_TRUNC)
                    ...
                    pivot_b = np.int16(np.percentile(res_b, val_AGR))
                    pivot_g = np.int16(np.percentile(res_g, val_AGR))
                    pivot_r = np.int16(np.percentile(res_r, val_AGR))                

                    corr_b = np.int16(np.int16(res_b) - np.int16(fd_b) + pivot_b)
                    corr_g = np.int16(np.int16(res_g) - np.int16(fd_g) + pivot_g)
                    corr_r = np.int16(np.int16(res_r) - np.int16(fd_r) + pivot_r)
                    ...
                    res_b = np.uint8(corr_b)
                    res_g = np.uint8(corr_g)
                    res_r = np.uint8(corr_r)                

                             
            # Contrast CLAHE openCV
            if flag_contrast_CLAHE ==1 :                    
                clahe = cv2.createCLAHE(clipLimit=val_contrast_CLAHE, tileGridSize=(8,8))
                res_b = clahe.apply(res_b)
                res_g = clahe.apply(res_g)
                res_r = clahe.apply(res_r)
            ...
            if flag_sharpen_soft1 == 1 or flag_unsharp_mask == 1 :
                ...
                img_g_gpu.free()
                img_b_gpu.free()
                image_traitee = cv2.merge((res_b,res_g,res_r))
            ...
            if flag_SAT == True :
                ...
                image_saturee = np.array(img_sat)
                image_saturee = cv2.GaussianBlur(image_saturee,(17,17),cv2.BORDER_DEFAULT,)               
                imggrey = cv2.cvtColor(image_traitee, cv2.COLOR_BGR2GRAY)
                res_b,res_g,res_r = cv2.split(image_saturee)
...
#        time_exec_test = (time.time()-start_time_test)*1000
#        labelInfo1.config(text = str(time_exec_test) + "                            ")

        if flag_DEMO == 1 :
            image_traitee[0:height,0:width//2] = image_initiale
...
def video_capture() :
    ...
    if nb_cap_video == 1 :
        ...
        if image_traitee.ndim == 3 :
            ...
            if flag_HQ == 0 :
                videoOut = cv2.VideoWriter(os.path.join(video_path,nom_video), fourcc, 25, (width, height), isColor = True) # video compressée
            else :
                videoOut = cv2.VideoWriter(os.path.join(video_path,nom_video), fourcc, 25, (width, height), isColor=True) # video RAW
        else :
            height,width = image_traitee.shape
            if flag_HQ == 0 :
                videoOut = cv2.VideoWriter(os.path.join(video_path,nom_video), fourcc, 25, (width, height), isColor = False) # video compressée
            else :
                videoOut = cv2.VideoWriter(os.path.join(video_path,nom_video), fourcc, 25, (width, height), isColor=False) # video RAW
        labelInfo1.config(text = "                                                                                                        ") 
...
def mode_acq_rapide() :
    global labelParam1,flag_acq_rapide,camera,exp_min, exp_max, exp_delta,exp_interval,echelle1,val_exposition,\
           frame_rate,flag_acquisition_en_cours,flag_stop_acquisition,exposition
    time.sleep(0.001)
 
def mode_acq_medium() :
    global labelParam1,flag_acq_rapide,camera,exp_min, exp_max, exp_delta,exp_interval,echelle1,\
           val_exposition,frame_rate,flag_acquisition_en_cours,flag_stop_acquisition,exposition
    time.sleep(0.001)
...
def valeur_exposition (event=None) :
    global timeout_val,flag_acq_rapide,camera,val_exposition,echelle1,val_resolution,flag_stop_acquisition,exposition
    time.sleep(0.001)
...
def choix_BIN1(event=None) :
    global val_FS,camera,val_resolution,echelle3,flag_acquisition_en_cours,flag_stop_acquisition,mode_BIN
    reset_general_FS()
    reset_FS()
    time.sleep(0.001)
...
def choix_resolution_camera(event=None) :
    global val_delai,val_FS,camera,traitement,val_resolution,res_cam_x,res_cam_y, img_cam,rawCapture,res_x_max,res_y_max,echelle3,\
           flag_image_disponible,flag_acquisition_en_cours,flag_stop_acquisition
    reset_FS()
    reset_general_FS()
    time.sleep(0.001)
...
def commande_noir_blanc() :
    global val_FS,flag_noir_blanc,format_capture,flag_stop_acquisition,flag_filtrage_ON
    reset_FS()
    reset_general_FS()
    time.sleep(0.001)
...
def commande_TRKSAT() :
    global flag_TRKSAT,calque_satellites
    if choix_TRKSAT.get() == 0 :
        flag_TRKSAT = 0
        cr,cg,cb = cv2.split(calque_satellites)
        cr = cr * 0
        cg = cg * 0
        cb = cb * 0
        calque_satellites = cv2.merge((cb,cg,cr))
...
def commande_TRKMET() :
    global flag_TRKMET,calque_met
    if choix_TRKMET.get() == 0 :
        flag_TRKMET = 0
        cr,cg,cb = cv2.split(calque_met)
        cr = cr * 0
        cg = cg * 0
        cb = cb * 0
        calque_met = cv2.merge((cb,cg,cr))
...
cadre = Frame (fenetre_principale, width = 1850 , heigh = 1060)
cadre.pack ()
...
xS1=1510 # Durée exposition en ms
yS1=325
...
echelle70.place(anchor="c", x=15,y=490)
...
echelle2 = Scale (cadre, from_ = 0, to = 510 , command= valeur_gain, orient=HORIZONTAL, length = 330, width = 10, resolution = 1, label="",showvalue=1,tickinterval=50,sliderlength=20)
...
echelle80 = Scale (cadre, from_ = 0, to = 10.0, command= choix_amplif, orient=HORIZONTAL, length = 280, width = 10, resolution = 0.05, label="",showvalue=1,tickinterval=1,sliderlength=20)
...
echelle14 = Scale (cadre, from_ = 0, to = 100, command= choix_w_red, orient=HORIZONTAL, length = 140, width = 10, resolution = 1, label="",showvalue=1,tickinterval=25,sliderlength=20)
...
echelle15 = Scale (cadre, from_ = 0, to = 100, command= choix_w_blue, orient=HORIZONTAL, length = 140, width = 10, resolution = 1, label="",showvalue=1,tickinterval=25,sliderlength=20)
...
echelle100 = Scale (cadre, from_ = 40, to = 160, command= choix_w_reds, orient=HORIZONTAL, length = 100, width = 10, resolution = 1, label="",showvalue=1,tickinterval=40,sliderlength=20)
...
echelle101 = Scale (cadre, from_ = 40, to = 160, command= choix_w_greens, orient=HORIZONTAL, length = 100, width = 10, resolution = 1, label="",showvalue=1,tickinterval=40,sliderlength=20)
...
echelle102 = Scale (cadre, from_ = 40, to = 160, command= choix_w_blues, orient=HORIZONTAL, length = 100, width = 10, resolution = 1, label="",showvalue=1,tickinterval=40,sliderlength=20)
...
CB2DC.place(anchor="w",x=1680, y=455)
...
# Choix filtre Denoise Paillou stacking AADP
CBEPFS = Checkbutton(cadre,text="Dn Adaptative Absorber", variable=choix_denoise_stacking_Paillou,command=commande_denoise_stacking_Paillou,onvalue = 1, offvalue = 0)
CBEPFS.place(anchor="w",x=1535, y=485)
# Stacking Mode
RBAADP1 = Radiobutton(cadre,text="High Dyn", variable=choix_dyn_AADP,command=choix_dyn_high,value=1)
RBAADP1.place(anchor="w",x=1660, y=485)
RBAADP2 = Radiobutton(cadre,text="Low Dyn", variable=choix_dyn_AADP,command=choix_dyn_low,value=2)
RBAADP2.place(anchor="w",x=1725, y=485)
...
CBEPF.place(anchor="w",x=1450, y=525)
...
echelle30.place(anchor="w", x=1510,y=525)
...
CBDS.place(anchor="w",x=1645, y=525)
...
echelle4.place(anchor="w", x=1710,y=525)
...
CBGR.place(anchor="w",x=1450, y=570)
...
echelle60.place(anchor="w", x=1530,y=575)

# Choix Parametre Atenuation Gradient Removal
labelParam61 = Label (cadre, text = "At")
labelParam61.place(anchor="e", x=1720,y=575)
...
echelle61.place(anchor="w", x=1720,y=575)

# Choix Parametre Niveau Gaussian Blur Gradient Removal
labelParam62 = Label (cadre, text = "Blur")
labelParam62.place(anchor="e", x=1635,y=620)
echelle62 = Scale (cadre, from_ = 0, to = 40, command= choix_NGB, orient=HORIZONTAL, length = 180, width = 10, resolution = 1, label="",showvalue=1,tickinterval=10,sliderlength=20)
echelle62.set(val_NGB)
echelle62.place(anchor="w", x=1640,y=620)


# Traitement histogram avant ou après filtres
#RBFL1 = Radiobutton(cadre,text="Front", variable=choix_FrontBack,command=choix_front,value=1)
#RBFL1.place(anchor="w",x=1450, y=650)
#RBFL2 = Radiobutton(cadre,text="Back", variable=choix_FrontBack,command=choix_back,value=2)
#RBFL2.place(anchor="w",x=1520, y=650)
...
CBIN.place(anchor="w",x=1450, y=675)
...
CBHE2.place(anchor="w",x=1520, y=675)
...
echelle16.place(anchor="w", x=1580,y=675)
...
echelle5.place(anchor="w", x=1580,y=720)

labelParam6 = Label (cadre, text = "Max") # choix valeur histogramme strech maximum
labelParam6.place(anchor="w", x=1700,y=720)
...
echelle6.place(anchor="w", x=1720,y=720)
...
echelle12.place(anchor="w", x=1580,y=765)

labelParam13 = Label (cadre, text = "Dec") # choix valeur histogramme Signoide param 2
labelParam13.place(anchor="w", x=1700,y=765)
...
echelle13.place(anchor="w", x=1720,y=765)
...
CBCC.place(anchor="w",x=1450, y=810)

labelParam9 = Label (cadre, text = "Clip") # choix valeur contrate CLAHE
labelParam9.place(anchor="w", x=1555,y=810)
echelle9 = Scale (cadre, from_ = 0.5, to = 4, command= choix_valeur_CLAHE, orient=HORIZONTAL, length = 240, width = 10, resolution = 0.1, label="",showvalue=1,tickinterval=1,sliderlength=20)
echelle9.set(val_contrast_CLAHE)
echelle9.place(anchor="w", x=1580,y=810)
...
CBHQC.place(anchor="w",x=1450, y=25)
...
echelle8.place(anchor="w", x=1570,y=855)
...
labelInfo1.place(anchor="w", x=1475,y=1000)
...
Button1 = Button (cadre,text = "Start CAP", command = start_pic_capture)
Button1.place(anchor="w", x=1475,y=855)

Button2 = Button (cadre,text = "Stop CAP", command = stop_pic_capture)
Button2.place(anchor="w", x=1475,y=880)

Button3 = Button (cadre,text = "Start REC", command = start_video_capture)
Button3.place(anchor="w", x=1475,y=915)

Button4 = Button (cadre,text = "Stop REC", command = stop_video_capture)
Button4.place(anchor="w", x=1475,y=940)


Button (fenetre_principale, text = "Quitter", command = quitter).place(x=1760,y=990, anchor="e")


cadre_image = Canvas (cadre, width = cam_displ_x, height = cam_displ_y, bg = "grey")
cadre_image.place(anchor="w", x=60,y=cam_displ_y/2+5)
