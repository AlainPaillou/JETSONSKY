...
# Titre programme
titre = "JetsonSky V14.10 Windows release - CUDA / OPENCV - Copyright Alain PAILLOU 2018-2022"
...
#                    res_bb, res_gb, res_rb = cv2.split(image_brut_blur)
                    image_brut_blur[:,:,0] = clahe.apply(image_brut_blur[:,:,0])
                    image_brut_blur[:,:,1] = clahe.apply(image_brut_blur[:,:,1])
                    image_brut_blur[:,:,2] = clahe.apply(image_brut_blur[:,:,2])
#                    image_brut_blur = cv2.merge((res_bb,res_gb,res_rb))
                    imag_brute = cv2.addWeighted(image_brut_CV, 0.65, image_brut_blur, 0.35, 0)
