...
# Titre programme
titre = "JetsonSky V14.05 jetson Linux release - CUDA / OPENCV - Copyright Alain PAILLOU 2018-2022"
...
mod = SourceModule("""
__global__ void Mono_staramp_GPU(unsigned char *dest_r, unsigned char *img_r,
unsigned char *grey_gpu, unsigned char *grey_blur_gpu,long int width, long int height, float val_Mu, float val_Ro, float val_ampl, float *Corr_GS)
{
  long int j = threadIdx.x + blockIdx.x * blockDim.x;
  long int i = threadIdx.y + blockIdx.y * blockDim.y;
...
Mono_staramp_GPU = mod.get_function("Mono_staramp_GPU")
    ...
    if flag_filtrage_ON == True :
        ...
        if flag_noir_blanc == 1 or flag_colour_camera == False :
            ...
            if flag_AmpSoft == 1 and flag_lin_gauss == 3 :                
                niveau_blur = 7
                imagegreyblur=cv2.GaussianBlur(imag_brute,(niveau_blur,niveau_blur),cv2.BORDER_DEFAULT)
                ...
                Mono_staramp_GPU(r_gpu, img_r_gpu, grey_gpu, grey_blur_gpu,np.int_(width), np.int_(height), \
