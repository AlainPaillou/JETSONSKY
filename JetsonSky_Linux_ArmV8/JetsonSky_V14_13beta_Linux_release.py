...
############################################################################################################
#                         Images/videos acquisition & treatment software for ZWO camera                    #
#                                                                                                          #
#                                                                                                          #
#     ASI178MC&MM, ASI290MC&MM, ASI224MC, ASI385MC, ASI482MC, ASI485MC, ASI585MC, ASI662MC, ASI678MC       #
#                                                                                                          #
#                                                                                                          #
...
# Titre programme
titre = "JetsonSky V14.13beta Jetson Linux release - CUDA / OPENCV - Copyright Alain PAILLOU 2018-2022"
...
sat_old_id  = np.zeros(1000,dtype=int)
...
//      vgrey = (int)(int(img_r[index] * 0.2126 + img_g[index] * 0.7152 + img_b[index] * 0.0722));
      dest_r[index] = (int)(min(max(int(img_r[index]*img_grey[index]/vgrey), 0), 255));
      dest_g[index] = (int)(min(max(int(img_g[index]*img_grey[index]/vgrey), 0), 255));
      dest_b[index] = (int)(min(max(int(img_b[index]*img_grey[index]/vgrey), 0), 255));
      img_r[index] = dest_r[index];
      img_g[index] = dest_g[index];
      img_b[index] = dest_b[index];
    } 
...
class acquisition(Thread) :
    ...
    def run(self) :
        global type_debayer,camera,nb_erreur,image_brute,flag_acquisition_en_cours,flag_autorise_acquisition,Date_hour_image,\
               ...
               flag_nouvelle_resolution,calque_stars,calque_satellites,calque_TIP,calque_direction_satellites
                ...
                if flag_hold_picture == 0 :
                    ...
                    try :
                        ...
                        if flag_nouvelle_resolution == True :
                            calque_stars = np.zeros_like(image_tmp)
                            calque_satellites = np.zeros_like(image_tmp)
                            calque_direction_satellites = np.zeros_like(image_tmp)
                            calque_TIP = np.zeros_like(image_tmp)
                            flag_nouvelle_resolution = False
...
def refresh() :
    global camera,traitement, img_cam,cadre_image,rawCapture,image_affichee,image_brut,flag_image_disponible,flag_quitter,\
           ...
           val_gain, echelle2,val_exposition,echelle1,imggrey1,imggrey2,flag_FDIF,FDIF,flag_TRKSAT,correspondance,\
           calque_stars,calque_satellites,calque_TIP,flag_nouvelle_resolution,nb_sat,sat_x,sat_y,sat_old_x,sat_old_y,calque_direction_satellites
    ...
    if flag_image_disponible == True :
        if flag_stop_acquisition == False :
            ...
            if flag_FDIF == 1 :
                stars_detection()
            if flag_TRKSAT == 1 and nb_sat < 10 :
...
def satellites_tracking ():
    global imggrey1,imggrey2,image_traitee,calque_satellites,calque_direction_satellites,flag_sat_exist,sat_x,sat_y,sat_s,sat_old_x,sat_old_y,sat_cntdwn,nb_sat
                ...
                if nb_sat >= 0 :
                    ...
                    for i in range(nb_sat+1) :
                        if correspondance[i] >=0:
                            ...
                            cv2.line(calque_satellites,start_point,end_point,(0,255,0),2)           
...
def stars_detection() :
    ...
    if flag_noir_blanc == 0 and flag_colour_camera == True :
        ...
        calque_stars = 0
        seuilb = np.percentile(image_traitee, 95)
        seuil_min_blob = seuilb + 25
        params = cv2.SimpleBlobDetector_Params()    
        params.minThreshold = seuil_min_blob;     # Change thresholds
        params.maxThreshold = 255;
        params.thresholdStep = 10 # steps to go through
        ...
        params.filterByInertia = False
        detector = cv2.SimpleBlobDetector_create(params)
        image_traitee_color = cv2.cvtColor(image_traitee,cv2.COLOR_GRAY2RGB)
        keypoints = detector.detect(image_traitee_color)
    for kp in keypoints:
    ...
    if flag_filtrage_ON == True :
        ...
        if flag_noir_blanc == 0 and flag_colour_camera == True:
            ...
            if flag_contrast_CLAHE ==1 :
                ...
                if flag_type_clahe == 2 :
                    clahe = cv2.createCLAHE(clipLimit=val_contrast_CLAHE, tileGridSize=(8,8))
                    image_brut_CV_colour=cv2.merge((res_b1,res_g1,res_r1))
                    image_brut_CV_grey= cv2.cvtColor(image_brut_CV_colour, cv2.COLOR_BGR2GRAY)
                    image_brut_grey_blur = cv2.GaussianBlur(image_brut_CV_grey,(3,3),cv2.BORDER_DEFAULT,)
                    image_brut_grey_blur = clahe.apply(image_brut_grey_blur)
                    ...
                    img_grey_gpu = drv.mem_alloc(res_r1.size * res_r1.dtype.itemsize)

                    drv.memcpy_htod(img_b_gpu, res_b1)  
                    drv.memcpy_htod(img_g_gpu, res_g1)
                    drv.memcpy_htod(img_r_gpu, res_r1)
                    drv.memcpy_htod(img_grey_gpu, image_brut_grey_blur)

                    Colour_clahe_GPU(r_gpu, g_gpu, b_gpu, img_r_gpu, img_g_gpu, img_b_gpu, img_grey_gpu,np.int_(width), np.int_(height), \
                            block=(nb_ThreadsX,nb_ThreadsY,1), grid=(nb_blocksX,nb_blocksY))
