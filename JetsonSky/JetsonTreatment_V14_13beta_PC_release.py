...
# Titre programme
titre = "Video Treatment V14.13beta Windows release - CUDA / OPENCV - Copyright Alain PAILLOU 2018-2022"
...
def refresh() :
    global val_nb_capt_video,flag_cap_video,image_brut_Base,camera,video,traitement, img_cam,cadre_image,rawCapture,image_affichee,image_brut_CV,flag_image_disponible,\
          ...
          imggrey1,imggrey2,flag_FDIF,FDIF,flag_TRKSAT,calque_direction_satellites,calque_stars,calque_satellites,flag_nouvelle_resolution,nb_sat,sat_x,sat_y,sat_old_x,sat_old_y,calque_met,\
          correspondance,vitesse,sat_id,sat_old_id,compteur_sat,old_sat,sat_speed
    ...
    if (video.isOpened()):
        ...
        if ret == True :
            ...
            if flag_nouvelle_resolution == True :
                calque_stars = np.zeros_like(image_brut_Base)
                calque_satellites = np.zeros_like(image_brut_Base)
                calque_met = np.zeros_like(image_brut_Base)
                calque_direction_satellites = np.zeros_like(image_brut_Base)
                flag_nouvelle_resolution = False
...
def application_filtrage() :
    ...
    imag_brute = image_brut_Base.copy()
    ...
    if flag_filtrage_ON == True :
        ...
        if flag_noir_blanc == 1 or flag_colour_camera == False :
            ...
            if flag_DEMO == 1 :
                image_traitee[0:height,0:width//2] = image_brut_Base[0:height,0:width//2]
