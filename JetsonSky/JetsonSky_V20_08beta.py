...
def video_capture() :
    ...
    if nb_cap_video == 1 :
        ...
        if flag_HQ == 1 :
            ...
            if image_traitee.ndim == 3 :
                height,width,layers = image_traitee.shape
                video = cv2.VideoWriter(path_video, fourcc, 25, (width, height), isColor=True) # video RAW
            else :
                height,width = image_traitee.shape
                video = cv2.VideoWriter(path_video, fourcc, 25, (width, height), isColor=False) # video RAW
        else :
            if image_traitee.ndim == 3 :
                height,width,layers = image_traitee.shape
                if Dev_system == "PC Windows" :
                    path_video_Windows = './Videos/' + nom_video
                    gst_pipe = "appsrc ! video/x-raw,format=BGR,width=" + str(width) + ",height=" + str(height) + ",framerate=25/1 ! queue ! videoconvert ! nvh264enc ! h264parse ! qtmux ! filesink location=" + path_video_Windows
            ...
            if  (not video.isOpened()):
                ...
                if image_traitee.ndim == 3 :
                    height,width,layers = image_traitee.shape
                    video = cv2.VideoWriter(path_video, fourcc, 25, (width, height), isColor = True) # video compressée
                else :
                    height,width = image_traitee.shape
                    video = cv2.VideoWriter(path_video, fourcc, 25, (width, height), isColor = False) # video compressée
        labelInfo1.config(text = "                                                                                                        ") 
...
CBOP.place(anchor="w",x=1730, y=280)
...
echelle9 = Scale (cadre, from_ = 0.1, to = 4, command= choix_valeur_CLAHE, orient=HORIZONTAL, length = 100, width = 10, resolution = 0.1, label="",showvalue=1,tickinterval=1,sliderlength=20)
...
labelParam109.place(anchor="w", x=1700,y=760)
echelle109 = Scale (cadre, from_ = 4, to = 48, command= choix_grid_CLAHE, orient=HORIZONTAL, length = 100, width = 10, resolution = 4, label="",showvalue=1,tickinterval=8,sliderlength=20)
echelle109.set(val_grid_CLAHE)
echelle109.place(anchor="w", x=1720,y=760)
