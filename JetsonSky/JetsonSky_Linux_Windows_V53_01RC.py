...
class acquisition(Thread) :
    ...
    def run(self) :
        global flag_16b,cupy_context,flag_autorise_acquisition,image_brute_expo,timeoutexp,image_captured,New_camera_image,flag_acquisition_thread_OK,nb_erreur
            ...
            while flag_acquisition_thread_OK == True :
                if flag_autorise_acquisition == True :
                    try :
                        ...
                        if flag_16b == True :
                            image_brute_expo = cp.asarray(image_camera_tmp,dtype=cp.uint16)
                        else :
                            image_brute_expo = cp.asarray(image_camera_tmp,dtype=cp.uint8)                        
                        image_captured = True
                    except Exception as error :
                        image_captured = False
                        New_camera_image = False
...
def camera_acquisition() :
    global type_debayer,camera,nb_erreur,image_brute,flag_autorise_acquisition,key_pressed,flag_stop_acquisition,\
            flag_image_disponible,flag_noir_blanc,cupy_context,delta_RX,delta_RY,delta_BX,delta_BY,mode_BIN,flag_HB,\
            timeoutexp,flag_filtrage_ON,flag_filtre_work,flag_IsColor,labelInfo10,image_captured,image_brute_expo,\
    ...
    with cupy_context :
        if image_captured == True :
            if New_camera_image == True :
                New_camera_image = False
                image_brute_cam = image_brute_expo.copy()
                ret = True
                if flag_16b == True :
                    image_brute_cam16 = cp.asarray(image_brute_cam,dtype=cp.uint16)
...
def refresh() :
    global video,flag_cap_video,camera,traitement,cadre_image,image_brute,flag_image_disponible,flag_quitter,timeoutexp,curFPS,fpsQueue,image_brut_read,delta_zx,delta_zy,flag_new_stab_window,\
           ...
           calque_stars,calque_satellites,calque_TIP,flag_nouvelle_resolution,nb_sat,sat_x,sat_y,sat_s,res_bb1,res_gg1,res_rr1,cupy_context,flag_TRIGGER,flag_filtrage_ON,flag_filtre_work,\
...
def application_filtrage_color(res_b1,res_g1,res_r1) :
    global compteur_FS,Im1OK,Im2OK,Im3OK,b1_sm, b2_sm, b3_sm, b4_sm, b5_sm, g1_sm, g2_sm, g3_sm, g4_sm, g5_sm, r1_sm, r2_sm, r3_sm, r4_sm, r5_sm,\
           ...
           val_heq2,val_SGR,val_NGB,val_AGR,flag_filtre_work,flag_AmpSoft,val_ampl,grad_vignet,compteur_AANR,compteur_AANRB,compteur_RV,flag_SAT,val_SAT,flag_NB_estime,TTQueue,curTT,\
...
def application_filtrage_mono(res_b1) :
    global compteur_FS,Im1OK,Im2OK,Im3OK,b1_sm, b2_sm, b3_sm, b4_sm, b5_sm,flag_IsColor,flag_BFReference,\
           ...
           val_heq2,val_SGR,val_NGB,val_AGR,flag_filtre_work,flag_AmpSoft,val_ampl,grad_vignet,compteur_AANR,compteur_RV,flag_SAT,val_SAT,flag_NB_estime,TTQueue,curTT,\
