...
#                                                                                                          #
#                                Using NVidia GPU computing - CUPY & CUDA                                  #
#                                                                                                          #
...
if my_os == "win32" :
    ...
    if keyboard_layout == "AZERTY" :
        ...
        titre = "JetsonSky " +  JetsonSky_version + " Windows release - CUDA / OPENCV / CUPY - ZSQD Stab - Arrows Zoom - ShiftR TGHFV - ShiftB OLMK; - Copyright Alain PAILLOU 2018-2024"
    else :
        print("Keyboard layout : QWERTY")
        titre = "JetsonSky " +  JetsonSky_version + " Windows release - CUDA / OPENCV / CUPY - WSAD Stab - Arrows Zoom - ShiftR TGHFV - ShiftB OL:K, - Copyright Alain PAILLOU 2018-2024"
...
if my_os == "linux" :
    ...
    if keyboard_layout == "AZERTY" :
        print("Keyboard layout : AZERTY")
        titre = "JetsonSky " +  JetsonSky_version + " Linux release - CUDA / OPENCV / CUPY - ZSQD Stab - Arrows Zoom - ShiftR TGHFV - ShiftB OLMK; - Copyright Alain PAILLOU 2018-2024"
    else :
        print("Keyboard layout : QWERTY")
        titre = "JetsonSky " +  JetsonSky_version + " Linux release - CUDA / OPENCV / CUPY - WSAD Stab - Arrows Zoom - ShiftR TGHFV - ShiftB OL:K, - Copyright Alain PAILLOU 2018-2024"
...
#os.add_dll_directory("D:\\Astro\\Soft\\opencv\\opencv-4.10.0\\build\\install\\x64\\vc17\\bin")
import cv2
try :
...
compteur_3FNR = 0
compteur_3FNRB = 0
GPU_BAYER = 1 #  0 : RAW   1 : RGGB   2 : BGGR    3 : GRBG     4 : GBRG
...
bayer_sensor = IntVar()
bayer_sensor.set(2) # Bayer sensor pattern
...
def init_camera() :
    global camera,flag_colour_camera,format_capture,controls,res_cam_x,res_cam_y, \
           ...
           RES_X_BIN2_1_1,RES_Y_BIN2_1_1,type_debayer,sensor_bits_depth
...
class acquisition(Thread) :
    ...
    def run(self) :
        global type_debayer,camera,nb_erreur,image_brute,flag_acquisition_en_cours,flag_autorise_acquisition,image_camera,key_pressed,\
               flag_image_disponible,flag_noir_blanc,flag_hold_picture,image_camera_old,s1,delta_RX,delta_RY,delta_BX,delta_BY,mode_BIN,flag_HB,\
               res_bb1,res_gg1,res_rr1,numero_image,timeoutexp,flag_new_image,flag_filtrage_ON,flag_filtre_work,flag_IsColor,labelInfo10,threshold_16bits,choix_hard_bin
                    ...
                    if flag_hold_picture == 0 and flag_HDR == False :
                        ...
                        try :
                            ...
                            if flag_filtrage_ON == False and flag_filtre_work == False :
                                res_bb1 = image_brute_cam
                            else :
                                if flag_noir_blanc == 0 and flag_colour_camera == True :
                                    res_r = cp.zeros_like(image_brute_cam,dtype=cp.uint8)
                                    res_g = cp.zeros_like(image_brute_cam,dtype=cp.uint8)
                                    res_b = cp.zeros_like(image_brute_cam,dtype=cp.uint8)
                                    img = image_brute_cam
                                ...
                                if flag_STAB == True and flag_HDR == False and flag_filtrage_ON == True :
                                    if Dim == 3 :
                                        image_brute = cupy_separateRGB_2_numpy_RGBimage(res_rr1,res_gg1,res_bb1)
...
#                                print("erreur acquisition : ",nb_erreur)
    ...
    def stop(self) :
        global flag_start_acquisition,flag_autorise_acquisition
...
def refresh() :
    global video,flag_cap_video,camera,traitement,cadre_image,image_brute,flag_image_disponible,flag_quitter,timeoutexp,flag_new_image,curFPS,fpsQueue,image_brut_read,delta_zx,delta_zy,flag_new_stab_window,\
           flag_acquisition_en_cours,flag_autorise_acquisition,flag_premier_demarrage,flag_BIN2,image_traitee,val_SAT,exposition,total_start,total_stop,flag_image_video_loaded,flag_BFReference,\
           ...
           delta_RX,delta_RY,delta_BX,delta_BY,track,track_crater_history,track_sat_history,track_sat,track_crater,key_pressed,DSW,echelle210,frame_position,max_frames,h,flag_new_frame_position
        ...
        if flag_camera_ok == True :
            ...
            if ((flag_image_disponible == True and flag_new_image == True) or flag_HDR == True) and (image_camera > numero_image):
                if flag_HDR == True and flag_filtrage_ON == True :
                    try :
                        if flag_16b == False :
                            image_tmp=camera.capture_video_frame_RAW8_NUMPY(filename=None,timeout=timeoutexp)
                            ...
                            image_camera_tmp = camera.capture_video_frame_RAW16_CUPY(filename=None,timeout=timeoutexp)
                            image_camera_base = image_camera_tmp.get()
                            
                            image_brute_cam16 = cp.asarray(image_camera_base,dtype=cp.uint16)
                            ...
                            image_brute = image_brute_cam_tmp.get()
                            image_brute1 = cv2.cvtColor(image_brute, type_debayer)
                            ...
                            image_brute_cam_tmp = cp.asarray(image_brute_cam8,dtype=cp.uint8)
                            image_brute = image_brute_cam_tmp.get()
                            image_brute2 = cv2.cvtColor(image_brute, type_debayer)
                            ...
                            image_brute_cam_tmp = cp.asarray(image_brute_cam8,dtype=cp.uint8)
                            image_brute = image_brute_cam_tmp.get()
                            image_brute3 = cv2.cvtColor(image_brute, type_debayer)
                            ...
                            image_brute_cam_tmp = cp.asarray(image_brute_cam8,dtype=cp.uint8)
                            image_brute = image_brute_cam_tmp.get()
                            image_brute4 = cv2.cvtColor(image_brute, type_debayer)
...
#                                    cv2.putText(calque_satellites_AI, f"{object_name}: {conf:.2f}", (x, y - 5), cv2.FONT_HERSHEY_PLAIN, 1, BOX_COLOUR, ep)
                    ...
                    if res_cam_x < int(1350*fact_s) and flag_full_res == 1 :
                        image_traitee_resize = cv2.resize(image_traitee,(cam_displ_x,cam_displ_y),interpolation = cv2.INTER_LINEAR)
                        cadre_image.im=PIL.Image.fromarray(image_traitee_resize)
                    ...
                    if res_cam_x <= 1350*fact_s and flag_full_res == 0 :
            ...
            if flag_image_mode == False :
                ...
                if flag_new_frame_position == False :
                    ...
                    flag_new_frame_position = False
                if video_frame_position >= video_frame_number :
...
def satellites_tracking_AI ():
    global imggrey12,imggrey22,image_traitee,flag_IsColor,s1

    if flag_IsColor == True :
        imggrey22 = cv2.cvtColor(image_traitee, cv2.COLOR_BGR2GRAY)
    else :
        imggrey22 = image_traitee
    try :
...
def satellites_tracking ():
    global imggrey1,imggrey2,image_traitee,calque_satellites,calque_direction_satellites,flag_sat_exist,sat_x,sat_y,sat_s,sat_old_x,sat_old_y,sat_cntdwn,nb_sat,sat_id,compteur_sat, \
           flag_first_sat_pass,nb_trace_sat,sat_speed,max_sat,flag_IsColor,sat_old_dx,sat_old_dy

    if flag_IsColor == True :
        imggrey2 = cv2.cvtColor(image_traitee, cv2.COLOR_BGR2GRAY)
    else :
        imggrey2 = image_traitee
...
def application_filtrage_color(res_b1,res_g1,res_r1) :
    global compteur_FS,Im1OK,Im2OK,Im3OK,compteur_images,numero_image,b1_sm, b2_sm, b3_sm, b4_sm, b5_sm, g1_sm, g2_sm, g3_sm, g4_sm, g5_sm, r1_sm, r2_sm, r3_sm, r4_sm, r5_sm,\
           Im4OK,Im5OK,flag_hold_picture,val_denoise,val_denoise_KNN,val_histo_min,val_histo_max,image_camera,image_camera_old,image_brute_grey,s1,BFREF_image,flag_BFREF_image, \
        ...
        if flag_filtrage_ON == True :
            ...
            if flag_IsColor == True :
                ...
                if flag_BFREFPT == True and flag_image_mode == False :
                    ...
                    crop_im_grey = cv2.cvtColor(crop_Im, cv2.COLOR_BGR2GRAY)
#                    img_qual_PT = cv2.Laplacian(crop_im_grey, cv2.CV_64F, ksize=laplacianksize).var()
...
def application_filtrage_mono(res_b1) :
    global compteur_FS,Im1OK,Im2OK,Im3OK,compteur_images,numero_image,b1_sm, b2_sm, b3_sm, b4_sm, b5_sm,flag_IsColor,flag_BFReference,\
           Im4OK,Im5OK,flag_hold_picture,val_denoise,val_denoise_KNN,val_histo_min,val_histo_max,image_camera,image_camera_old,image_brute_grey,s1, BFREF_image_PT,max_qual_PT,flag_BFREF_image_PT,\
...
def choix_BIN1(event=None) :
    global val_FS,camera,val_resolution,echelle3,flag_acquisition_en_cours,flag_stop_acquisition,mode_BIN,flag_nouvelle_resolution,flag_TIP,choix_TIP,flag_cap_video,flag_image_disponible,flag_new_image
...
def choix_BIN2(event=None) :
    global val_FS,camera,val_resolution,echelle3,flag_acquisition_en_cours,flag_stop_acquisition,mode_BIN,flag_nouvelle_resolution,flag_TIP,choix_TIP,flag_cap_video
...
def reset_general_FS():
    global val_FS,compteur_FS,Im1OK,Im2OK,Im3OK,Im4OK,Im5OK,stack_div,echelle20,choix_AADF,choix_AADFB,flag_first_sat_pass,nb_sat,\
           ...
           flag_AI_Craters,track_crater_history,flag_AI_Satellites,choix_AI_Satellites,track_satelitte_history,model_craters_track,model_satellites_track,flag_image_disponible,flag_new_image
    flag_image_disponible = False
    flag_new_image = False
    val_FS = 1
    compteur_FS = 0
...
def choix_position_frame(event=None) :
    global frame_position, echelle210,video,flag_new_frame_position
    if flag_cap_video == False and flag_image_mode == False :
        flag_new_frame_position = True
        frame_position = echelle210.get()
        video.set(cv2.CAP_PROP_POS_FRAMES,frame_position)
...
def calcul_AZ_HT_cible(jours_obs, mois_obs, annee_obs, heure_obs, min_obs, second_obs, lat_obs, long_obs, cible_ASD, cible_DEC):
    ...
    if sinazimut > 0 :
        ...
        azimut_cible = - math.acos(cosazimut)
# azimut_cible et hauteur cible retournees en radian
...
# declinaison de la cible cible retournee en radian, ascension droite retourneee en horaire decimal
...
def commande_AI_Satellites() :
    global flag_AI_Satellites,model_Satellites,track_satellite_history
...
CBHDR.place(anchor="w",x=1590+delta_s, y=80)
...
CBHDR.place(anchor="w",x=1630+delta_s, y=80)


# Resolution setting
labelParam3 = Label (cadre, text = "RES : ")
labelParam3.place(anchor="w", x=1680+delta_s,y=80)
echelle3 = Scale (cadre, from_ = 1, to = 9, command= choix_resolution_camera, orient=HORIZONTAL, length = 120, width = 7, resolution = 1, label="",showvalue=1,tickinterval=1,sliderlength=20)
echelle3.set (val_resolution)
echelle3.place(anchor="w", x=xS3+delta_s,y=yS3)
