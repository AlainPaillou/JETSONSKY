...
def init_camera() :
    global camera,flag_colour_camera,format_capture,controls,res_cam_x,res_cam_y, \
           ...
           RES_X_BIN2_1_1,RES_Y_BIN2_1_1,type_debayer
...
class acquisition(Thread) :
    ...
    def run(self) :
        global type_debayer,camera,nb_erreur,image_brute,flag_acquisition_en_cours,flag_autorise_acquisition,image_camera,key_pressed,\
               flag_image_disponible,flag_noir_blanc,flag_hold_picture,image_camera_old,s1,delta_RX,delta_RY,delta_BX,delta_BY,\
               res_bb1,res_gg1,res_rr1,numero_image,timeoutexp,flag_new_image,flag_filtrage_ON,flag_filtre_work,flag_IsColor,labelInfo10
                    ...
                    if flag_hold_picture == 0 and flag_HDR == False :
                        ...
                        try :
                            ...
                            if flag_filtrage_ON == False and flag_filtre_work == False :
                                res_bb1 = cp.asarray(image_brute_cam,dtype=cp.uint8)
                            else :
...
labelParam50.place(anchor="w", x=15,y=250)
...
if flag_camera_ok == True :
    ...
    CBMNT.place(anchor="w",x=5, y=190)
...
echelle100 = Scale (cadre, from_ = 80, to = 120, command= choix_w_reds, orient=HORIZONTAL, length = 360, width = 7, resolution = 0.1, label="",showvalue=1,tickinterval=10,sliderlength=20)
...
echelle101 = Scale (cadre, from_ = 80, to = 120, command= choix_w_greens, orient=HORIZONTAL, length = 360, width = 7, resolution = 0.1, label="",showvalue=1,tickinterval=10,sliderlength=20)
...
echelle102 = Scale (cadre, from_ = 80, to = 120, command= choix_w_blues, orient=HORIZONTAL, length = 360, width = 7, resolution = 0.1, label="",showvalue=1,tickinterval=10,sliderlength=20)
...
if flag_camera_ok == False :
    ...
    if Video_Test.lower().endswith(('.mp4', '.avi', '.ser', '.mov')) :
        ...
        flag_SAT_Image = False
        Sat_Vid_Img.set(0)
#        echelle210 = Scale (cadre, from_ = 0, to = max_frames, command= choix_position_frame, orient=HORIZONTAL, length = 1350*fact_s, width = 7, resolution = 1, label="",showvalue=1,tickinterval=100,sliderlength=20)
#        echelle210.set(frame_position)
#        echelle210.place(anchor="w", x=60,y=h-30)
