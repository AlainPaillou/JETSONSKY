...
RES_Y_BIN2_16_9 = [1040,960,720,600,480,384,300]
...
def refresh() :
    global video,flag_cap_video,camera,traitement,cadre_image,image_brute_base,flag_image_disponible,flag_quitter,timeoutexp,flag_new_image,curFPS,fpsQueue,image_brut_read,\
           ...
           calque_stars,calque_satellites,calque_TIP,flag_nouvelle_resolution,nb_sat,sat_x,sat_y,sat_old_x,sat_old_y,res_bb1,res_gg1,res_rr1,image_camera_old,s1,flag_TRIGGER,\
           calque_direction_satellites,correspondance,vitesse,sat_id,sat_old_id,compteur_sat,old_sat,sat_speed,compteur_images,numero_image,nb_erreur,image_brute_grey,flag_sat_detected,\
           start_time_video,stop_time_video,time_exec_test,total_time,font,image_camera_old,image_camera,res_cam_x,res_cam_y,flag_premier_demarrage,Video_Test,TTQueue,curTT,max_sat,\
           video_frame_number,video_frame_position
...
#                        alignMTB.process(img_list, img_list)
                ...
                if flag_stop_acquisition == False and flag_filtre_work == False :
                    ...
                    if flag_DETECT_STARS == 1 and flag_noir_blanc == 0 and flag_colour_camera == True :
                        stars_detection()
                    if flag_TRKSAT == 1 and nb_sat < max_sat and flag_noir_blanc == 0 and flag_colour_camera == True :
                        size = 0.5
...
#                        image_traitee = cv2.addWeighted(image_traitee, 1, calque_direction_satellites, 1, 0)     
            ...
            if flag_image_video_loaded == True :
                ...
                if flag_DETECT_STARS == 1 and flag_noir_blanc == 0 and flag_colour_camera == True and flag_image_mode == False :
                    stars_detection()
                if flag_TRKSAT == 1 and nb_sat < max_sat and nb_sat >= 0 and flag_noir_blanc == 0 and flag_colour_camera == True and flag_image_mode == False :
                    font = cv2.FONT_HERSHEY_SIMPLEX
                    size = 0.5
                    for i in range(nb_sat+1):
                        if correspondance[i] >=0:
    ...
    with s1 :     
        if flag_filtrage_ON == True :
            ...
            if flag_noir_blanc == 0 and flag_colour_camera == True:
                ...
                if flag_SAT == True :
                    colour_base_image = cv2.merge((res_r1.get(),res_g1.get(),res_b1.get()))
                    res_b4,res_g4,res_r4 = gaussianblur_colour(res_r1,res_g1,res_b1,7)
                    colour_base_blur = cv2.merge((res_r4.get(),res_g4.get(),res_b4.get()))               
                    img_base=PIL.Image.fromarray(colour_base_blur)
                    if flag_torchvision == True :
                        img_sat = F.adjust_saturation(img_base,val_SAT)
                        if ImageNeg == 1 :
                            img_sat = F.adjust_hue(img_sat,0.5)
                    else :
                        converter = ImageEnhance.Color(img_base)
                        img_sat = converter.enhance(val_SAT)

                    res_r1,res_g1,res_b1=cupy_RGBImage_2_cupy_separateRGB(cp.array(img_sat))
