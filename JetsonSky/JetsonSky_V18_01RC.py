...
class acquisition(Thread) :
    ...
    def run(self) :
        global type_debayer,camera,nb_erreur,image_brute,flag_acquisition_en_cours,flag_autorise_acquisition,Date_hour_image,\
               ...
               flag_sub_dark,choix_BGSUB,flag_BG_OK,flag_sub_BG,image_BG,numero_image,echelle1,flag_acq_rapide,timeoutexp
...
def refresh() :
    global camera,traitement, img_cam,cadre_image,rawCapture,image_affichee,image_brute,flag_image_disponible,flag_quitter,timeoutexp,\
    ...
    if (flag_image_disponible == True and np.any(res_bb1) > 0) or flag_HDR == True:
        if flag_HDR == True :
            try :
