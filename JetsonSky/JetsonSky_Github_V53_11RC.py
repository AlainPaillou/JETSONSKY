...
def refresh() :
    global video,flag_cap_video,camera,traitement,cadre_image,image_brute,flag_image_disponible,flag_quitter,timeoutexp,curFPS,fpsQueue,image_brut_read,delta_zx,delta_zy,flag_new_stab_window,\
           ...
           curFPS,SER_depth,flag_SER_file,res_cam_x_base,res_cam_y_base,flag_capture_image_reference,Image_Reference,flag_image_reference_OK,previous_frame_number,flag_blur_image_ref_sub,flag_birds
                ...
                if ret_img == True :
                    ...
                    if flag_AI_Craters == True and flag_crater_model_loaded == True:
                        ...
                        if flag_AI_Trace == True :
                            result_craters2 = model_craters_track.track(image_model, device=0, half=True, conf=0.05, persist=True, verbose=False)[0]
                        else :
                            result_craters2 = model_craters_predict.predict(image_model, device=0, max_det=100, half=True, verbose=False)[0]
                        ...
                        if flag_sat_OK == True :
                            try :
                                if flag_AI_Trace == True :
                                    ...
                                    result_sat2 = model_satellites_track.track(image_model, tracker=Custom_satellites_model_tracker, device=0, half=False, conf=0.01, persist=True, verbose=False)[0]
                                else :
                                    result_sat2 = model_satellites_predict.predict(image_model, device=0, max_det=100, half=True, verbose=False)[0]
                                model_OK = True
                            except :
                                model_OK = False
                            if model_OK == True :
                    ...
                    if flag_display == True :
                        if flag_full_res == 0 :
                            if res_cam_x > int(1350*fact_s) :
                                # OPT: image_traitee est déjà en RAM CPU à ce stade.
                                # Upload GPU→CPU→GPU juste pour un resize d'affichage est plus lent que CPU seul.
                            ...
                            if res_cam_x < int(1350*fact_s) :
                                # OPT: idem — CPU resize suffisant pour l'affichage
                                image_traitee_resize = cv2.resize(image_traitee, (cam_displ_x, cam_displ_y), interpolation=cv2.INTER_LINEAR)
                                cadre_image.im=PIL.Image.fromarray(image_traitee_resize)
                            else :                   
                                old_dzx = delta_zx
...
def satellites_tracking ():
    global imggrey1,imggrey2,image_traitee,calque_satellites,calque_direction_satellites,flag_sat_exist,sat_x,sat_y,sat_s,sat_old_x,sat_old_y,sat_cntdwn,nb_sat,sat_id,compteur_sat, \
           ...
           img_sat_buf5,flag_img_sat_buf1,flag_img_sat_buf2,flag_img_sat_buf3
