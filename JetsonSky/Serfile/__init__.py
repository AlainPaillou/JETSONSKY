...
class Serfile():
    ...
    def readTrailFromHeader(self):
        ...
        trail = []
        offset = np.int64(self._header['ImageHeight']) * np.int64(self._header['ImageWidth']) * np.int64(self._header['FrameCount']) * np.int64(self._bytesPerPixels) + np.int64(178)
    ...
    def _readExistingHeader(self):
        ...
        try : 
            with open(self._nameOfSerfile, 'rb') as file:
                ...
                lengthWithoutTrail = np.int64(ImageHeight) * np.int64(ImageWidth) * np.int64(FrameCount) * np.int64(BytesPerPixel) + np.int64(178)
                lengthWithTrail = np.int64(ImageHeight) * np.int64(ImageWidth) * np.int64(FrameCount) * np.int64(BytesPerPixel) + np.int64(178) + np.int64(8) * np.int64(FrameCount) #SPECIFICATION : trail contain 1 date per frame.
                readOK = True
                trail = False
    ...
    def readFrameAtPos(self,n):
        ...
        if n<self._length : 
            with open(self._nameOfSerfile, 'rb') as file:
                frame = np.array([])
                offset = np.int64(178) + np.int64(n) * np.int64(np.multiply(self._frameDimension, self._bytesPerPixels, dtype=object))
                file.seek(offset)
