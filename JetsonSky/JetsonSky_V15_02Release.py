...
if flag_torch == True and flag_torchvision == True :
    flag_pytorch = True
    print("PyTorch libray OK")
else :
    flag_pytorch = False
...
def refresh() :
    global camera,traitement, img_cam,cadre_image,rawCapture,image_affichee,image_brut,flag_image_disponible,flag_quitter,\
           ...
           val_gain, echelle2,val_exposition,echelle1,imggrey1,imggrey2,flag_FDIF,FDIF,flag_TRKSAT,correspondance,labelInfo2,\
...
def gaussianblur_mono(image_mono,niveau_blur):
    ...
    if compute_library == "opencv" or compute_library == "pytorch" :
...
def gaussianblur_colour(im_b,im_g,im_r,niveau_blur):
    ...
    if compute_library == "opencv" or compute_library == "vpi" :
    ...
    if compute_library == "pytorch" :
        colour_image = cv2.merge((im_b,im_g,im_r))  
        transform = tv.transforms.Compose([tv.transforms.ToTensor()])
        torch_image = transform(colour_image)
        torch_image = torch_image.to('cuda')
        gaussianblur = tv.transforms.GaussianBlur(kernel_size=(niveau_blur, niveau_blur), sigma=(0.1, 10))
        gaussian_image_color = gaussianblur(torch_image)
        gaussian_image_color = gaussian_image_color.cpu()
        image_gaussian_blur_colour = gaussian_image_color.detach().numpy()
        image_gaussian_blur_colour=np.swapaxes(image_gaussian_blur_colour,0,1)
        image_gaussian_blur_colour=np.swapaxes(image_gaussian_blur_colour,1,2)        
        im_GB_b = image_gaussian_blur_colour[:,:,0]
        im_GB_g = image_gaussian_blur_colour[:,:,1]
        im_GB_r = image_gaussian_blur_colour[:,:,2]
...
def image_negative_colour (red,green,blue):
    ...
    if compute_library == "pytorch" :
        res_b1_torch = torch.from_numpy(blue)
        res_g1_torch = torch.from_numpy(green)
        res_r1_torch = torch.from_numpy(red)
        res_b1_torch = res_b1_torch.to('cuda')
        res_g1_torch = res_g1_torch.to('cuda')
        res_r1_torch = res_r1_torch.to('cuda')
        res_b1_torch = torch.bitwise_not(res_b1_torch)
        res_g1_torch = torch.bitwise_not(res_g1_torch)
        res_r1_torch = torch.bitwise_not(res_r1_torch)
        res_b1_torch = res_b1_torch.cpu()
        res_g1_torch = res_g1_torch.cpu()
        res_r1_torch = res_r1_torch.cpu()
        blue = res_b1_torch.numpy()
        green = res_g1_torch.numpy()
        red = res_r1_torch.numpy()
...
def application_filtrage() :
    global image_traitee1,image_traitee2,image_traitee3,image_traitee4,image_traitee5,compteur_FS,Im1OK,Im2OK,Im3OK,\
           ...
           flag_dyn_AADP,Corr_GS,calque_TIP,azimut,hauteur,flag_type_clahe,val_ghost_reducer,res_bb1,res_gg1,res_rr1,res_b2,res_g2,res_r2
    ...
    if flag_filtrage_ON == True :            
        ...
        if flag_noir_blanc == 0 and flag_colour_camera == True:
            ...
            height,width,layers = image_brute.shape
            ...
            if flag_GR == True :
                if grad_vignet == 1 :
                    ...
                    fd_b=gaussianblur_mono(fd_b,niveau_blur) 
                    fd_g=gaussianblur_mono(fd_g,niveau_blur) 
                    fd_r=gaussianblur_mono(fd_r,niveau_blur)
            ...
            if flag_contrast_CLAHE ==1 :
                ...
                if flag_type_clahe == 2 :
                    ...
                    image_brut_grey_blur = gaussianblur_mono(image_brut_CV_grey,3)
            ...
            if flag_SAT == True :
                img_base=PIL.Image.fromarray(cv2.merge((res_b1,res_g1,res_r1)))
                converter = ImageEnhance.Color(img_base)
                img_sat = converter.enhance(val_SAT)
                image_saturee = np.array(img_sat)
                image_saturee = gaussianblur_mono(image_saturee,17)               
                imggrey = cv2.cvtColor(cv2.merge((res_b1,res_g1,res_r1)), cv2.COLOR_BGR2GRAY)
...
def choix_library_pytorch(event=None):
    global compute_library,type_library
    if flag_pytorch == True :
        compute_library = "pytorch"
        print("PYTORCH")
    else :
        compute_library = "opencv"
        type_library.set(1)
...
labellibraries.place (anchor="w",x=5, y=900)
RBTL1 = Radiobutton(cadre,text="OpenCV", variable=type_library,command=choix_library_opencv,value=1)
RBTL1.place(anchor="w",x=5, y=920)
RBTL2 = Radiobutton(cadre,text="PyTorch", variable=type_library,command=choix_library_pytorch,value=2)
RBTL2.place(anchor="w",x=5, y=940)
RBTL3 = Radiobutton(cadre,text="VPI", variable=type_library,command=choix_library_vpi,value=3)
RBTL3.place(anchor="w",x=5, y=960)
