...
class Camera(object):
    ...
    def capture_video_frame_RAW8_NUMPY(self, buffer_=None, filename=None, timeout=None):
    ...
    def capture_video_frame_RAW16_NUMPY(self, buffer_=None, filename=None, timeout=None):
        """Capture a single frame from video. Type :class:`numpy.ndarray`"""
        data = self.get_video_data(buffer_=buffer_, timeout=timeout)
        whbi = self.get_roi_format()
        shape = [whbi[1], whbi[0]]
    ...
    def capture_video_frame_RAW8_CUPY(self, buffer_=None, filename=None, timeout=None):
        """Capture a single frame from video. Type :class:`cupy.ndarray`"""
        data = self.get_video_data(buffer_=buffer_, timeout=timeout)
        whbi = self.get_roi_format()
        shape = [whbi[1], whbi[0]]
        img = cp.frombuffer(data, dtype=cp.uint8)
        img = img.reshape(shape)
        return img

    def capture_video_frame_RAW16_CUPY(self, buffer_=None, filename=None, timeout=None):
        """Capture a single frame from video. Type :class:`cupy.ndarray`"""
        data = self.get_video_data(buffer_=buffer_, timeout=timeout)
        whbi = self.get_roi_format()
        shape = [whbi[1], whbi[0]]
        img = cp.frombuffer(data, dtype=cp.uint16)
        img = img.reshape(shape)
        return img
