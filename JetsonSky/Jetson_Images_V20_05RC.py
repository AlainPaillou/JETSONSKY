...
############################################################################################################
#                                                                                                          #
#                               Images treatment software for ZWO cameras                                  #
#                                                                                                          #
#                                                                                                          #
...
if my_os == "win32" :
    ...
    titre = "JetsonImages " +  Image_version + " Windows RC - CUDA / OPENCV / CUPY / VPI - Copyright Alain PAILLOU 2018-2023"

if my_os == "linux" :
    Dev_system = "Jetson Linux"
    print("Linux system")
    titre = "JetsonImages " +  Image_version + " Linux RC - CUDA / OPENCV / CUPY / VPI - Copyright Alain PAILLOU 2018-2023" 
...
def refresh() :
    global camera,traitement, img_cam,cadre_image,rawCapture,image_affichee,image_brut,flag_image_disponible,flag_quitter,\
           ...
           val_gain, echelle2,val_exposition,echelle1,imggrey1,imggrey2,flag_FDIF,FDIF,image_brute,image_brut_read
...
def pic_capture() :
    ...
    if flag_HQ == 0 :
        filesimages = [('JPG image', '*.jpg')]
    else :
        filesimages = [('TIF image', '*.tif')]
    nom_fichier = asksaveasfilename(filetypes = filesimages, defaultextension = filesimages)
    if nom_fichier != "" :
        if image_traitee.ndim == 3 :
            if flag_HQ == 0 :
                cv2.imwrite(nom_fichier, cv2.cvtColor(image_traitee, cv2.COLOR_BGR2RGB), [int(cv2.IMWRITE_JPEG_QUALITY), 95]) # JPEG file format
            else :
                cv2.imwrite(nom_fichier, cv2.cvtColor(image_traitee, cv2.COLOR_BGR2RGB)) # TIFF file format
        else :
            if flag_HQ == 0 :
                cv2.imwrite(nom_fichier, image_traitee, [int(cv2.IMWRITE_JPEG_QUALITY), 95]) # JPEG file format
            else :
                cv2.imwrite(nom_fichier, image_traitee) # TIFF file format
        labelInfo1.config(text = "capture "+ nom_fichier)
    flag_cap_pic = False
...
def load_picture() :
    global image_brut_read,flag_image_disponible,image_brute
    ...
    Image_Test = fd.askopenfilename(title='Open a file',initialdir=image_path,filetypes=filetypes)
    if Image_Test != "" :
        image_brut_read = cv2.imread(Image_Test,cv2.IMREAD_COLOR)
        image_brut_read = cv2.cvtColor(image_brut_read,cv2.COLOR_BGR2RGB)
        image_brut_Base = image_brut_read
...
CBHQC.place(anchor="w",x=1450, y=910)

labelInfo1 = Label (cadre, text = text_info1) # label info n°1
labelInfo1.place(anchor="w", x=1550,y=930)
...
Button1 = Button (cadre,text = "SAVE PIC", command = start_pic_capture,padx=10,pady=0)
Button1.place(anchor="w", x=1460,y=855)

Button2 = Button (cadre,text = "LOAD PIC", command = load_picture,padx=10,pady=0)
