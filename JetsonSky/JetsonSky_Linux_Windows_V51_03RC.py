                        ...
                        try :
                            ...
                            if flag_filtrage_ON == False and flag_filtre_work == False :
                                ...
                                if flag_noir_blanc == 0 and flag_colour_camera == True :
                                    res_rr1 = cp.zeros_like(image_brute_cam,dtype=cp.uint8)
                                    res_gg1 = cp.zeros_like(image_brute_cam,dtype=cp.uint8)
                                    res_bb1 = cp.zeros_like(image_brute_cam,dtype=cp.uint8)
                                    img = image_brute_cam
                                    ...
                                    if  flag_hot_pixels == True :
                                        ...
                                        Dead_Pixels_Remove_Colour_GPU((nb_blocksX,nb_blocksY),(nb_ThreadsX,nb_ThreadsY),(res_rr1, img,  np.intc(width), np.intc(height), np.intc(Pixel_threshold), np.intc(GPU_BAYER)))
                                        img = res_rr1.copy()
                                    nb_blocksX = ((width // 2) // nb_ThreadsX) + 1
                                    nb_blocksY = ((height // 2) // nb_ThreadsY) + 1
                                    Image_Debayer_GPU((nb_blocksX,nb_blocksY),(nb_ThreadsX,nb_ThreadsY),(res_rr1, res_gg1, res_bb1, img, np.intc(width), np.intc(height), np.intc(GPU_BAYER)))
                                    if key_pressed != "" :
                                    ...
                                    if delta_RX !=0 or delta_RY !=0 or delta_BX !=0 or delta_BY != 0 :
                                        ...
                                        nb_blocksX = (width // nb_ThreadsX) + 1
                                        nb_blocksY = (height // nb_ThreadsY) + 1
                                        RGB_Align_GPU((nb_blocksX,nb_blocksY),(nb_ThreadsX,nb_ThreadsY),(res_rr1, res_gg1, res_bb1, img_r, img_g, img_b, np.intc(width), np.intc(height), np.intc(delta_RX), np.intc(delta_RY), np.intc(delta_BX), np.intc(delta_BY)))
                                    Dim = 3
                                    flag_IsColor = True
                                else :
                                    Dim = 1
...
def refresh() :
    global video,flag_cap_video,camera,traitement,cadre_image,image_brute,flag_image_disponible,flag_quitter,timeoutexp,flag_new_image,curFPS,fpsQueue,image_brut_read,delta_zx,delta_zy,flag_new_stab_window,\
           ...
           curFPS,SER_depth,flag_SER_file,res_cam_x_base,res_cam_y_base
                ...
                if flag_HDR == True and flag_filtrage_ON == True :
                    try :
                        if flag_16b == False :
                            ...
                            thres2 = 2 ** (TH_16B + delta_th * 2)
                            thres1 = 2 ** (TH_16B + delta_th * 3)
...
def satellites_tracking_AI ():
    global imggrey12,imggrey22,image_traitee,flag_IsColor,s1,sat_frame_count,sat_frame_count_AI,flag_img_sat_buf1_AI,flag_img_sat_buf2_AI,flag_img_sat_buf3_AI,\
