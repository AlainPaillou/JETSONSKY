...
def refresh() :
    global video,flag_cap_video,camera,traitement,cadre_image,image_brute_base,flag_image_disponible,flag_quitter,timeoutexp,flag_new_image,curFPS, fpsQueue,image_brut_read,\
           ...
           val_gain, echelle2,val_exposition,echelle1,imggrey1,imggrey2,flag_FDIF,FDIF,flag_TRKSAT,labelInfo2,Date_hour_image,image_camera,val_nb_capt_video,flag_image_mode,\
...
#                                texte = "Satellite No " + str(sat_id[i])
            ...
            if flag_image_video_loaded == True :
                ...
                if flag_TRKSAT == 1 and flag_noir_blanc == 0 and flag_colour_camera == True and flag_image_mode == False :
                    satellites_tracking()
                if flag_FDIF == 1 and flag_noir_blanc == 0 and flag_colour_camera == True and flag_image_mode == False :
                    stars_detection()
                if flag_TRKSAT == 1 and nb_sat < 10 and flag_noir_blanc == 0 and flag_colour_camera == True and flag_image_mode == False :
                    font = cv2.FONT_HERSHEY_SIMPLEX
                    size = 0.5
                    for i in range(nb_sat+1):
                        if correspondance[i] >=0:
