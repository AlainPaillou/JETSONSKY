...
class acquisition(Thread) :
    ...
    def run(self) :
        global type_debayer,camera,nb_erreur,image_brute_base,flag_acquisition_en_cours,flag_autorise_acquisition,image_camera,\
               ...
               res_bb1,res_gg1,res_rr1,flag_sub_dark,numero_image,timeoutexp,flag_new_image
...
#                                image_brute = cv2.subtract(image_brute,Master_Dark)
                            if flag_noir_blanc == 0 and flag_colour_camera == True :
                                ...
                                rn = r.download()
                                flag_image_disponible = False
                                res_bb1 = cp.asarray(bn)
                                res_gg1 = cp.asarray(gn)
                                res_rr1 = cp.asarray(rn)
                            else :
                                if type_flip == "vertical" :
                                    image_brute = cv2.cuda.flip(image_brute,0)
                                if type_flip == "horizontal" :
                                    image_brute = cv2.cuda.flip(image_brute,1)
                                if type_flip == "both" :
                                    image_brute = cv2.cuda.flip(image_brute,-1)
                                flag_image_disponible = False
...
def refresh() :
    global video,flag_cap_video,camera,traitement,cadre_image,image_brute_base,flag_image_disponible,flag_quitter,timeoutexp,flag_new_image,\
           ...
           start_time_video,stop_time_video,time_exec_test,total_time,font,image_camera_old,image_camera,res_cam_x,res_cam_y,flag_premier_demarrage,Video_Test
    ...
    if flag_camera_ok == True :
        ...
        if (video.isOpened()):
            ...
            if ret == True :
                ...
                if type_flip == "vertical" :
                    res_bb1 = cv2.flip(res_bb1,0)
                    res_gg1 = cv2.flip(res_gg1,0)
                    res_rr1 = cv2.flip(res_rr1,0)
                if type_flip == "horizontal" :
                    res_bb1 = cv2.flip(res_bb1,1)
                    res_gg1 = cv2.flip(res_gg1,1)
                    res_rr1 = cv2.flip(r,1)
                if type_flip == "both" :
                    res_bb1 = cv2.flip(res_bb1,-1)
                    res_gg1 = cv2.flip(res_gg1,-1)
                    res_rr1 = cv2.flip(res_rr1,-1)
                res_bb1 = cp.asarray(res_bb1)
                res_gg1 = cp.asarray(res_gg1)
                res_rr1 = cp.asarray(res_rr1)
                flag_image_disponible = True
                application_filtrage(res_bb1,res_gg1,res_rr1)
                total_stop = cv2.getTickCount()
                total_time= int((total_stop-total_start)/cv2.getTickFrequency()*1000)
...
def application_filtrage(res_b1,res_g1,res_r1) :
    global compteur_FS,Im1OK,Im2OK,Im3OK,compteur_images,numero_image,b1_sm, b2_sm, b3_sm, b4_sm, b5_sm, g1_sm, g2_sm, g3_sm, g4_sm, g5_sm, r1_sm, r2_sm, r3_sm, r4_sm, r5_sm,\
           Im4OK,Im5OK,flag_hold_picture,val_denoise,val_denoise_KNN,val_histo_min,val_histo_max,image_camera,image_camera_old, \
           ...
           flag_dyn_AADP,Corr_GS,azimut,hauteur,flag_type_clahe,val_ghost_reducer,res_b2,res_g2,res_r2,time_exec_test,flag_HDR
    ...
    if flag_filtrage_ON == True :
        ...
        if flag_noir_blanc == 0 and flag_colour_camera == True:
            ...
            if flag_AmpSoft == 1 and flag_lin_gauss == 3 :
                ...
                Colour_2_Grey_GPU((nb_blocksX,nb_blocksY),(nb_ThreadsX,nb_ThreadsY),(r_gpu, res_r1, res_g1, res_b1, np.int_(width), np.int_(height)))
...
echelle70 = Scale (cadre, from_ = 0, to = 12, command= choix_val_SAT, orient=VERTICAL, length = 120, width = 7, resolution = 0.01, label="",showvalue=1,tickinterval=None,sliderlength=10)
...
# Choix Sharpen
CBSS1 = Checkbutton(cadre,text="Sharpen Val/Sigma", variable=choix_sharpen_soft1,command=commande_sharpen_soft1,onvalue = 1, offvalue = 0)
CBSS1.place(anchor="w",x=1450, y=400)
echelle152 = Scale (cadre, from_ = 0, to = 4, command= choix_val_sharpen, orient=HORIZONTAL, length = 120, width = 7, resolution = 0.1, label="",showvalue=1,tickinterval=40,sliderlength=20)
echelle152.set(val_sharpen)
echelle152.place(anchor="w", x=1560,y=400)
...
echelle153.place(anchor="w", x=1690,y=400)
