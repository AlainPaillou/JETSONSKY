...
#                                                                                                          #
#                                Using NVidia GPU computing - CUDA & CUPY - OCL                            #
#                                                                                                          #
#                                   Copyright Alain Paillou 2018-2026                                      #
#                                                                                                          #
...
JetsonSky_version = "V53_21RC"

# ============================================================
# OPTIMIZED BUILD — applied optimizations:
#   1. deque(maxlen=10) for fpsQueue / TTQueue      → O(1) pop
#   2. Vectorized NumPy for all LUT init/recompute  → no Python loops
#   3. Corr_GS_gpu / Corr_CLL_gpu pre-uploaded GPU  → no per-frame upload
#   4. HDR_compute: full GPU pipeline               → -8 GPU↔CPU round-trips
#   5. Image reference subtract on GPU              → -2 GPU↔CPU round-trips
#   6. AANR brightness scale: direct CuPy ops       → removed redundant cp.asarray()
#   7. Single YOLO inference (craters + satellites) → -2 model calls/frame
#   8. Display resize: CPU cv2.resize               → removed useless CUDA upload
#   9. stream=cupy_context on all 58 RawKernel calls→ async GPU pipeline
#  10. cupy_context.synchronize() before .get()     → single barrier per frame
# ============================================================
...
# Support 16 bits SER file with HDR mode on a single captured frame
...
if my_os == "win32" :
    ...
    if keyboard_layout == "AZERTY" :
        ...
        titre = "JetsonSky " +  JetsonSky_version + " Windows release - CUDA / CUPY / OPENCV - ZSQD Stab - Arrows Zoom - ShiftR TGHFV - ShiftB OLMK; - Copyright Alain PAILLOU 2018-2026"
    else :
        print("Keyboard layout : QWERTY")
        titre = "JetsonSky " +  JetsonSky_version + " Windows release - CUDA / CUPY / OPENCV - WSAD Stab - Arrows Zoom - ShiftR TGHFV - ShiftB OL:K, - Copyright Alain PAILLOU 2018-2026"
...
try :
    ...
    print("Cupy library Loaded")
    flag_cupy = True
except :
    flag_cupy = False
    print("Cupy library not loaded")
...
frame_position = 0
mode_HDR = "Inverse Percentile"
HDR_Percentile_Value = 100
val_3FNR_Thres = 0.5
...
beta = 1.0
alpha = 5
# OPT: calcul vectorisé NumPy (évite la boucle Python)
_x_cont = np.arange(255, dtype=np.float32)
Corr_cont = ((1 - np.exp(-_x_cont / alpha) + beta) / (1 + beta)).astype(np.float32)

quality = np.zeros(258, dtype=float)
tmp_qual = np.zeros(258, dtype=float)

# quality[1:258] déjà à 0 par np.zeros
max_quality = 1
quality_pos = 1

    
# OPT: vectorisé NumPy
trsf_r = np.arange(256, dtype=int)
trsf_g = np.arange(256, dtype=int)
trsf_b = np.arange(256, dtype=int)

# OPT: pré-upload sur GPU une seule fois — évite cp.asarray() à chaque frame dans la boucle chaude
Corr_GS = np.ones(255, dtype=np.float32)
Corr_GS_gpu = cp.asarray(Corr_GS)

Corr_CLL = np.zeros(256,dtype=np.uint8)
val_MuCLL = 0
val_RoCLL = 0.5
val_AmpCLL = 1.0
# OPT: calcul vectorisé NumPy de la LUT
_x_cll = np.arange(256, dtype=np.float32)
_Corr_tmp = np.exp(-0.5 * ((_x_cll * 0.0392157 - val_MuCLL) / val_RoCLL) ** 2)
_Corr_cll_f = _x_cll / (1.0 + val_AmpCLL * _Corr_tmp)
Corr_CLL = np.maximum.accumulate(np.clip(_Corr_cll_f, 0, 255).astype(np.uint8))
# OPT: pré-upload GPU une seule fois
Corr_CLL_gpu = cp.asarray(Corr_CLL, dtype=cp.uint8)
...
curFPS = 0.0
# OPT: deque(maxlen) → pop(0) O(1) au lieu de O(n) sur liste
from collections import deque as _deque
fpsQueue = _deque(maxlen=10)

curTT = 0.0
TTQueue = _deque(maxlen=10)
...
mode_HDR_select.set(3) # Initialisation du mode HDR Mertens
...
choix_Birds = IntVar()
choix_Birds.set(0) # Initialisation Birds daylight detection
...
HDR_compute_GPU2 = cp.RawKernel(r'''
extern "C" __global__
void HDR_compute_GPU2_C(unsigned char *dest_r, unsigned short int *img_r, long int width, long int height, float thres1, float thres2, float thres3, float thres4, char method, char BIN_mode, char Hard_BIN, char percentile_value)
{
  ...
  if (i < height && j < width) {
      ...
      if (image_brute_16 > (thres1 * 0.9f)) {
          over = image_brute_16 - (thres1 * 0.9f);
          image_brute_16 = (thres1 * 0.9f) + (over / (1.0f + (over / (thres1 * 0.1f))));
      }
      if (BIN_mode == 2) {
          if (Hard_BIN == 0) {
      ...
      image_brute_16 = img_r[index];
      if (image_brute_16 > (thres2 * 0.9f)) {
          over = image_brute_16 - (thres2 * 0.9f);
          image_brute_16 = (thres2 * 0.9f) + (over / (1.0f + (over / (thres2 * 0.1f))));
      }
      if (BIN_mode == 2) {
          if (Hard_BIN == 0) {
      ...
      image_brute_16 = img_r[index];
      if (image_brute_16 > (thres3 * 0.9f)) {
          over = image_brute_16 - (thres3 * 0.9f);
          image_brute_16 = (thres3 * 0.9f) + (over / (1.0f + (over / (thres3 * 0.1f))));
      }
      if (BIN_mode == 2) {
          if (Hard_BIN == 0) {
      ...
      image_brute_16 = img_r[index];
      if (image_brute_16 > (thres4 * 0.9f)) {
          over = image_brute_16 - (thres4 * 0.9f);
          image_brute_16 = (thres4 * 0.9f) + (over / (1.0f + (over / (thres4 * 0.1f))));
      }
      if (BIN_mode == 2) {
          if (Hard_BIN == 0) {
    ...
    if (method == 0) {
        pixel_sum = image_brute1 + image_brute2 + image_brute3 + image_brute4;
        pixel_class = (0.00062476*pixel_sum*pixel_sum + 0.3617625*pixel_sum + 1.0) / 1020.0f;
        pixel_class = (min(max(pixel_class, 0.01f), 0.99f));
        class_percentile = 1.0f - (percentile_value / 100.0f) * pixel_class;
    
        float idx_virtuel = class_percentile * 3.0f;
        int idx_inf = __float2int_rd(idx_virtuel);
        int idx_sup = __float2int_ru(idx_virtuel);
        float fraction = idx_virtuel - idx_inf;

        float val_inf = (idx_inf == 0) ? image_brute1 : ((idx_inf == 1) ? image_brute2 : ((idx_inf == 2) ? image_brute3 : image_brute4));
        float val_sup = (idx_sup == 0) ? image_brute1 : ((idx_sup == 1) ? image_brute2 : ((idx_sup == 2) ? image_brute3 : image_brute4));

        dest_r[index] = (int)(val_inf + fraction * (val_sup - val_inf));
    }
    ...
    if (method == 2) {
        class_percentile = percentile_value / 100.0f;
...
extern "C" __global__
void HDR_compute_GPU_C(unsigned char *dest_r, unsigned short int *img_r, long int width, long int height, float thres_16b, char method, char BIN_mode, char Hard_BIN, char percentile_value)
{
...
B16_to_B8_GPU = cp.RawKernel(r'''
extern "C" __global__
void B16_to_B8_GPU_C(unsigned char *dest_r, unsigned short int *img_r, long int width, long int height)
{
  ...
  if (i < height && j < width) {  
    dest_r[index] = img_r[index] / 256;
    dest_r[index] = (int)(min(max(dest_r[index], 0), 256));
    } 
}
''', 'B16_to_B8_GPU_C')
...
def start_mount() :
    global flag_acquisition_mount,thread_2,flag_autorise_acquisition,flag_stop_acquisition
...
def opencv_color_debayer(image,debayer_pattern,cuda_flag) :
    ...
    if cuda_flag == False :
        tmpbase = cv2.UMat(image)
        tmprsz = cv2.cvtColor(tmpbase, debayer_pattern)
        debayer_image = tmprsz.get()
    else :
...
def HDR_compute(mono_colour,image_16b,method,threshold_16b,BIN_mode,Hard_BIN,type_bayer) :
    ...
    # OPT: rester entièrement sur GPU — suppression des 8 aller-retours GPU↔CPU (.get() + cp.asarray())
    def _hdr_exposure(src, thres):
        tmp = src.copy()
        tmp[tmp > thres] = thres
        if BIN_mode == 2 and Hard_BIN == False :
            cam8 = cp.clip(tmp / thres * 255.0 * 4.0, 0, 255).astype(cp.uint8)
        else :
            cam8 = cp.clip(tmp / thres * 255.0, 0, 255).astype(cp.uint8)
        return cam8

    image_brute1_gpu = _hdr_exposure(image_16b, thres1)
    image_brute2_gpu = _hdr_exposure(image_16b, thres2)
    image_brute3_gpu = _hdr_exposure(image_16b, thres3)
    image_brute4_gpu = _hdr_exposure(image_16b, thres4)

    if method == "Mertens" :
        # Mertens requiert des images CPU (OpenCV), transfert unique ici
        img_list = [image_brute1_gpu.get(), image_brute2_gpu.get(),
                    image_brute3_gpu.get(), image_brute4_gpu.get()]
        merge_mertens = cv2.createMergeMertens()
        res_mertens = merge_mertens.process(img_list)
        image_brute_cp = cp.clip(cp.asarray(res_mertens, dtype=cp.float32) * 255, 0, 255).astype(cp.uint8)
    if method == "Median" :
        img_stack = cp.stack([image_brute1_gpu, image_brute2_gpu, image_brute3_gpu, image_brute4_gpu], axis=0)
        image_brute_cp = cp.median(img_stack, axis=0).astype(cp.uint8)
    if method == "Mean" :
        img_stack = cp.stack([image_brute1_gpu, image_brute2_gpu, image_brute3_gpu, image_brute4_gpu], axis=0)
        image_brute_cp = cp.mean(img_stack, axis=0).astype(cp.uint8)
    if method == "Percentile" :
        img_stack = cp.stack([image_brute1_gpu, image_brute2_gpu, image_brute3_gpu, image_brute4_gpu], axis=0)
        image_brute_cp = cp.percentile(img_stack, HDR_Percentile_Value, axis=0).astype(cp.uint8)
...
def camera_acquisition() :
    global camera,nb_erreur,flag_autorise_acquisition,key_pressed,flag_stop_acquisition,GPU_BAYER,\
           ...
           New_camera_image,my_os,image_Demo
        ...
        if image_captured == True :
            if New_camera_image == True :
                ...
                if flag_16b == True :
                    ...
                    if flag_DEMO == 1 :
                        image_brute_cam8 = (image_brute_cam16 / 255.0)
                        image_brute_cam8 = cp.clip(image_brute_cam8,0,255)
                        image_brute_cam8_np = image_brute_cam8.get()
                        image_brute_demo = np.asarray(image_brute_cam8_np,dtype=np.uint8)
                        image_Demo = cv2.cvtColor(image_brute_demo,type_debayer)
                        if flag_noir_blanc == 1 and flag_colour_camera == True :
                            image_Demo = cv2.cvtColor(image_Demo,cv2.COLOR_BGR2GRAY)
                    ...
                    if flag_image_ref_sub == True and flag_image_reference_OK == True :
                        try :
                            # OPT: soustraction directement sur GPU, suppression du round-trip CPU
                            Image_Reference_gpu = cp.asarray(Image_Reference, dtype=cp.int16)
                            image_brute_cam = cp.clip(
                                image_brute_cam.astype(cp.int16) - Image_Reference_gpu, 0, 255
                            ).astype(cp.uint8)
                        except :
                            pass
                else :
...
def refresh() :
    global video,flag_cap_video,camera,traitement,cadre_image,image_brute,flag_image_disponible,flag_quitter,timeoutexp,curFPS,fpsQueue,image_brut_read,delta_zx,delta_zy,flag_new_stab_window,\
           ...
           nb_erreur,image_brute_grey,flag_sat_detected,start_time_video,stop_time_video,time_exec_test,total_time,font,flag_colour_camera,flag_iscolor,image_brute_camera,\
           ...
           flag_blur_image_ref_sub,flag_birds,image_Demo,flag_16b
            ...
            if flag_stop_acquisition == False :
                    ...
                    try :
                        if flag_16b == False :
                            ...
                            if type_debayer != 0 and flag_IsColor == True:
                                ...
                                mono_colour = "Mono"
                            if my_os == "linux" :
                                image_camera_base = cp.asarray(image_brute_camera,dtype=cp.uint16)
                            else :
                                image_camera_base = image_brute_camera
                            res_r = cp.asarray(image_camera_base,dtype=cp.uint8)
                            height,width = image_camera_base.shape
                            nb_blocksX = (width // nb_ThreadsX) + 1
                            nb_blocksY = (height // nb_ThreadsY) + 1
                            if flag_DEMO == 1 :
                                B16_to_B8_GPU((nb_blocksX,nb_blocksY),(nb_ThreadsX,nb_ThreadsY),(res_r, image_camera_base,  np.intc(width), np.intc(height)))
                                image_Demo = res_r.get()
                                image_Demo = opencv_color_debayer(image_Demo,type_debayer,flag_OpenCvCuda)
                                if mono_colour == "Mono" :
                                    image_Demo = cv2.cvtColor(image_Demo, cv2.COLOR_BGR2GRAY)
                            if mode_HDR != "Mertens" :
                                if mode_HDR == "Inverse Percentile" :
                                    type_method = 0
                                    HDR_Value = HDR_Percentile_Value
                                if mode_HDR == "Mean" :
                                    type_method = 1
                                    HDR_Value = 0
                                if mode_HDR == "Percentile" :
                                    type_method = 2
                                    HDR_Value = HDR_Percentile_Value
                                if flag_HB == True :
                                    Hard_BIN = 1
                                ...
                                HDR_compute_GPU2((nb_blocksX,nb_blocksY),(nb_ThreadsX,nb_ThreadsY),(res_r, image_camera_base,  np.intc(width), np.intc(height), np.float32(thres1) ,np.float32(thres2), np.float32(thres3), np.float32(thres4), \
                                            np.intc(type_method), np.intc(mode_BIN), np.intc(Hard_BIN),np.intc(HDR_Value)))
                ...
                if ret_img == True :
                    ...
                    if flag_AI_Craters == True and flag_crater_model_loaded == True:
                        ...
                        # OPT: un seul appel YOLO (l'ancien code appelait le modèle deux fois)
                        if flag_AI_Trace == True :
                            result_craters2 = model_craters_track.track(image_model, device=0, quantize=8, conf=0.05, persist=True, verbose=False)[0]
                        else :
                            result_craters2 = model_craters_predict.predict(image_model, device=0, max_det=100, quantize=8, verbose=False)[0]
                        boxes_crater = result_craters2.boxes.xywh.cpu()
                        bboxes_crater = np.asarray(result_craters2.boxes.xyxy.cpu(), dtype="int")
                        classes_crater = np.asarray(result_craters2.boxes.cls.cpu(), dtype="int")
...
#                            cv2.putText(image_traitee, f"{object_name}: {conf:.2f}", (x, y - 5), cv2.FONT_HERSHEY_PLAIN, 2, BOX_COLOUR, 2)
                    if flag_AI_Satellites == True and flag_satellites_model_loaded == True :
                        ...
                        if flag_sat_OK == True :
                            try :
                                if flag_AI_Trace == True :
                                    # OPT: un seul appel YOLO satellites (l'ancien code appelait deux fois)
                                    result_sat2 = model_satellites_track.track(image_model, tracker=Custom_satellites_model_tracker, device=0, quantize=8, conf=0.01, persist=True, verbose=False)[0]
                                else :
                                    result_sat2 = model_satellites_predict.predict(image_model, device=0, max_det=100, quantize=8, verbose=False)[0]
                                model_OK = True
                            except :
                                model_OK = False
                            if model_OK == True :
                                boxes_sat = result_sat2.boxes.xywh.cpu()
                                bboxes_sat = np.asarray(result_sat2.boxes.xyxy.cpu(), dtype="int")
                                classes_sat = np.asarray(result_sat2.boxes.cls.cpu(), dtype="int")
...
#                                    cv2.putText(calque_satellites_AI, f"{object_name}: {conf:.2f}", (x, y - 5), cv2.FONT_HERSHEY_PLAIN, 1, BOX_COLOUR, ep)
                    if flag_filtrage_ON == True :
                        if flag_CONST == 0 :
                            if ((flag_TRKSAT == 1 or flag_birds == 1) and flag_image_mode == False) and flag_REMSAT == 0 :
                                satellites_tracking()
                            ...
                            if ((flag_TRKSAT == 1 or flag_birds == 1)  and nb_sat >= 0 and nb_sat < max_sat and flag_image_mode == False) and flag_REMSAT == 0 :
                                flag_sat_detected = True
                                font = cv2.FONT_HERSHEY_SIMPLEX
                                size = 0.5
                                if flag_TRKSAT == 1 :
                                    texte1 = "Sat"
                                ...
                                for i in range(nb_sat+1):
                                    if correspondance[i] >=0:
                                        ...
                                        if flag_IsColor == True :
                                            cv2.circle(calque_direction_satellites, centercircle, radius, (0,255,0), 1, cv2.LINE_AA)
                                            cv2.putText(calque_direction_satellites, texte1,center_texte, font, size, (0, 255, 0), 1, cv2.LINE_AA)
                                            ...
                                            cv2.circle(calque_direction_satellites, centercircle, radius, (255,255,255), 1, cv2.LINE_AA)
                                            cv2.putText(calque_direction_satellites, texte1,center_texte, font, size, (255, 255, 255), 1, cv2.LINE_AA)
                                            center_texte = (sat_x[i]+10,sat_y[i]+25)
                                            texte = "Rel Speed " + str(sat_speed[i])
                    ...
                    # OPT: deque(maxlen=10) gère la limite automatiquement
                    ...
                    if flag_display == True :
                        if flag_full_res == 0 :
                            if res_cam_x > int(1350*fact_s) :
                                image_traitee_resize = cv2.resize(image_traitee, (cam_displ_x, cam_displ_y), interpolation=Interpolation_method)
            ...
            if flag_premier_demarrage == True :
                ...
                if flag_image_mode == False :
                    ...
                    if flag_SER_file == False :
                        video = cv2.VideoCapture(Video_Test, cv2.CAP_FFMPEG, [cv2.CAP_PROP_HW_ACCELERATION, cv2.VIDEO_ACCELERATION_ANY])
                ...
                if flag_SER_file == False :
                    ...
                    if SER_depth == 16 :
                        if flag_HDR == True and flag_filtrage_ON == True and type_debayer > 0 : 
                            image_brute_cp = cp.asarray(image_brute,dtype=cp.uint16)
                            if flag_DEMO == 1 :
                            ...
                            if mode_HDR != "Mertens" :
                                ...
                                thres2 = 2**(TH_16B + delta_th * 2) - 1;
                                thres1 = 2**(TH_16B + delta_th * 3) - 1;
                                HDR_compute_GPU2((nb_blocksX,nb_blocksY),(nb_ThreadsX,nb_ThreadsY),(res_r, image_brute_cp,  np.intc(width), np.intc(height), np.float32(thres1) ,np.float32(thres2), np.float32(thres3), np.float32(thres4), \
                                            np.intc(type_method), np.intc(mode_BIN), np.intc(Hard_BIN),np.intc(HDR_Value)))
                                image_brute = res_r.get()
                                ...
                                image_brute = HDR_compute(mono_colour,image_brute_cp,mode_HDR,TH_16B,mode_BIN,flag_HB,type_debayer)
                            if flag_noir_blanc == 1 :
                                if image_brute.ndim == 3 :
                                    image_brute = cv2.cvtColor(image_brute, cv2.COLOR_BGR2GRAY)
                                    flag_IsColor = False
...
#                                    cv2.putText(calque_satellites_AI, f"{object_name}: {conf:.2f}", (x, y - 5), cv2.FONT_HERSHEY_PLAIN, 1, BOX_COLOUR, ep)
                ...
                if (res_cam_x > int(cam_displ_x*fact_s) or res_cam_y > int(cam_displ_y*fact_s)) and flag_full_res == 0 :
                    image_traitee_resize = cv2.resize(image_traitee,(int(cam_displ_x*fact_s),int(cam_displ_y*fact_s)),interpolation = Interpolation_method)
                    cadre_image.im=PIL.Image.fromarray(image_traitee_resize)
...
def satellites_tracking_AI ():
    global imggrey12,imggrey22,image_traitee,flag_IsColor,cupy_context,sat_frame_count,sat_frame_count_AI,flag_img_sat_buf1_AI,flag_img_sat_buf2_AI,flag_img_sat_buf3_AI,\
           flag_first_sat_pass_AI,img_sat_buf1_AI,img_sat_buf2_AI,img_sat_buf3_AI
    ...
    if flag_img_sat_buf3_AI == True :
        img_sat_buf3_AI = image_traitee
        if flag_IsColor == True :
            imggrey22 = opencv_color_debayer(img_sat_buf3_AI, cv2.COLOR_BGR2GRAY,flag_OpenCvCuda)
            imggrey12 = opencv_color_debayer(img_sat_buf1_AI, cv2.COLOR_BGR2GRAY,flag_OpenCvCuda)
        else :
            imggrey22 = img_sat_buf3_AI
...
def satellites_tracking ():
    global imggrey1,imggrey2,image_traitee,calque_satellites,calque_direction_satellites,flag_sat_exist,sat_x,sat_y,sat_s,sat_old_x,sat_old_y,sat_cntdwn,nb_sat,sat_id,compteur_sat, \
           ...
           img_sat_buf5,flag_img_sat_buf1,flag_img_sat_buf2,flag_img_sat_buf3,detector_sat
    ...
    if flag_img_sat_buf3 == True :
        img_sat_buf3 = image_traitee
        if flag_IsColor == True :
            imggrey2 = opencv_color_debayer(img_sat_buf3, cv2.COLOR_BGR2GRAY,flag_OpenCvCuda)
            imggrey1 = opencv_color_debayer(img_sat_buf1, cv2.COLOR_BGR2GRAY,flag_OpenCvCuda)
        else :
            imggrey2 = img_sat_buf3
        ...
        if flag_birds == 1 :
            diff = cv2.subtract(imggrey1,imggrey2)
        else :
        ...
        if flag_first_sat_pass == True :
            ...
            params_sat.minDistBetweenBlobs = 4
            params_sat.filterByArea = True    # Filter by Area.
            if flag_birds == 1 :
                params_sat.minArea = 8
            else :
                params_sat.minArea = 3
            params_sat.maxArea = 3000
...
def stars_detection(draw) :
    global image_traitee,calque_stars,stars_x,stars_y,stars_s,nb_stars,flag_IsColor,flag_stars_detect_first_start,detector
    ...
    if seuil_min_blob > 160 :
        seuil_min_blob = 160
    if flag_stars_detect_first_start == True :
        ...
        flag_stars_detect_first_start = False
...
def reconstruction_image() :
    global imggrey1,imggrey2,image_traitee,sat_x,sat_y,sat_s,stars_x,stars_y,stars_s,nb_sat,flag_stars_detect_first_start, \
...
def application_filtrage_color(res_b1,res_g1,res_r1) :
    global compteur_FS,Im1OK,Im2OK,Im3OK,b1_sm, b2_sm, b3_sm, b4_sm, b5_sm, g1_sm, g2_sm, g3_sm, g4_sm, g5_sm, r1_sm, r2_sm, r3_sm, r4_sm, r5_sm,\
           ...
           imgb21,imgg21,imgr21,imgb22,imgg22,imgr22,imgb23,imgg23,imgr23,imgb21B,imgg21B,imgr21B,imgb22B,imgg22B,imgr22B,imgb23B,imgg23B,imgr23B,image_Demo
    ...
    with cupy_context :     
        if flag_filtrage_ON == True :
            ...
            if flag_IsColor == True :
                ...
                if flag_DEMO == 1 :
                    if flag_16b == True or SER_depth == 16 :
                        image_base = image_Demo
                    else :   
                ...
                if (val_reds != 1.0 or val_greens != 1.0 or val_blues != 1.0) :              
                    ...
                    if flag_TRSF == 1 :
                        # OPT: opérations vectorisées NumPy
                        if val_reds != 0 :
                            trsf_r = np.clip((trsf_r * val_reds).astype(int), 0, 255)
                        if val_greens != 0 :
                            trsf_g = np.clip((trsf_g * val_greens).astype(int), 0, 255)
                        if val_blues != 0 :
                            trsf_b = np.clip((trsf_b * val_blues).astype(int), 0, 255)
                ...
                if ImageNeg == 1 :
                    ...
                    if flag_TRSF == 1 :
                        # OPT: vectorisé NumPy
                        trsf_r = np.clip(256 - trsf_r, 0, 255)
                        trsf_g = np.clip(256 - trsf_g, 0, 255)
                        trsf_b = np.clip(256 - trsf_b, 0, 255)
                ...
                if flag_AANR == True and flag_image_mode == False :
                    ...
                    if Im2fsdnOK == True :
                        ...
                        # OPT: r_gpu/g_gpu/b_gpu sont déjà des arrays CuPy, cp.asarray() inutile
                        res_r1 = cp.clip(r_gpu.astype(cp.float32) * 1.05, 0, 255).astype(cp.uint8)
                        res_g1 = cp.clip(g_gpu.astype(cp.float32) * 1.05, 0, 255).astype(cp.uint8)
                        res_b1 = cp.clip(b_gpu.astype(cp.float32) * 1.05, 0, 255).astype(cp.uint8)
                ...
                if (flag_histogram_stretch == 1 or flag_histogram_equalize2 == 1 or flag_histogram_phitheta == 1) :
                    ...
                    if flag_TRSF == 1 :                                                      
                        if flag_histogram_phitheta == 1 :
                            # OPT: vectorisé NumPy
                            _sig = 255.0 / (1.0 + np.exp(-val_phi * ((trsf_r[1:] - val_theta) / 32.0)))
                            trsf_r[1:] = np.clip(_sig.astype(int), 0, 255)
                            _sig = 255.0 / (1.0 + np.exp(-val_phi * ((trsf_g[1:] - val_theta) / 32.0)))
                            trsf_g[1:] = np.clip(_sig.astype(int), 0, 255)
                            _sig = 255.0 / (1.0 + np.exp(-val_phi * ((trsf_b[1:] - val_theta) / 32.0)))
                            trsf_b[1:] = np.clip(_sig.astype(int), 0, 255)

                        if flag_histogram_equalize2 == 1 :
                            # OPT: vectorisé NumPy
                            trsf_r[1:] = np.clip((255.0 * np.power(trsf_r[1:] / 255.0, val_heq2)).astype(int), 0, 255)
                            trsf_g[1:] = np.clip((255.0 * np.power(trsf_g[1:] / 255.0, val_heq2)).astype(int), 0, 255)
                            trsf_b[1:] = np.clip((255.0 * np.power(trsf_b[1:] / 255.0, val_heq2)).astype(int), 0, 255)

                        if flag_histogram_stretch == 1 :
                            # OPT: vectorisé NumPy
                            delta_histo = val_histo_max - val_histo_min
                            trsf_r[1:] = np.clip(((trsf_r[1:] - val_histo_min) * (255.0 / delta_histo)).astype(int), 0, 255)
                            trsf_g[1:] = np.clip(((trsf_g[1:] - val_histo_min) * (255.0 / delta_histo)).astype(int), 0, 255)
                            trsf_b[1:] = np.clip(((trsf_b[1:] - val_histo_min) * (255.0 / delta_histo)).astype(int), 0, 255)
                    

                # Amplification soft Linear or Gaussian
                if flag_AmpSoft == 1 :
                    if flag_lin_gauss == 1 or flag_lin_gauss == 2 :
                        correction = Corr_GS_gpu  # OPT: pré-uploadé sur GPU
                    ...
                    if flag_TRSF == 1 :
                        # OPT: vectorisé NumPy
                        trsf_r = np.clip((trsf_r * val_ampl).astype(int), 0, 255)
                        trsf_g = np.clip((trsf_g * val_ampl).astype(int), 0, 255)
                        trsf_b = np.clip((trsf_b * val_ampl).astype(int), 0, 255)
                ...
                # Contrast Low Light
                if flag_CLL == 1 :
                    correction_CLL = Corr_CLL_gpu  # OPT: pré-uploadé sur GPU
                ...
                if flag_contrast_CLAHE ==1 :
                    if flag_OpenCvCuda == True :
                        ...
                        srcb = cv2.UMat(res_b1.get())
                        resb = clahe.apply(srcb)
                        srcg = cv2.UMat(res_g1.get())
                        resg = clahe.apply(srcg)
                        srcr = cv2.UMat(res_r1.get())
                        resr = clahe.apply(srcr)
                        res_b1 = cp.asarray(resb.get())
                        res_g1 = cp.asarray(resg.get())
                        res_r1 = cp.asarray(resr.get())
                ...
                if flag_reverse_RB == 0 :
                    # OPT: synchroniser le stream non-bloquant avant le transfert GPU→CPU final
                    cupy_context.synchronize()
...
def application_filtrage_mono(res_b1) :
    global compteur_FS,Im1OK,Im2OK,Im3OK,b1_sm, b2_sm, b3_sm, b4_sm, b5_sm,flag_IsColor,flag_BFReference,\
           ...
           compteur_3FNR2,img1_3FNR2OK,img2_3FNR2OK,img3_3FNR2OK,FNR2_First_Start,flag_3FNR2,compteur_3FNR2B,img1_3FNR2OKB,img2_3FNR2OKB,img3_3FNR2OKB,FNR2B_First_Start,flag_3FNR2B,\
           imgb21,imgb22,imgb23,imgb21B,imgb22B,imgb23B,image_Demo
            ...
            if flag_IsColor == False :
                ...
                if flag_AANR == True and flag_image_mode == False :
                    ...
                    if Im2fsdnOK == True :
                        ...
                        res_b2 = res_b1.copy()
                        # OPT: r_gpu est déjà CuPy, cp.asarray() inutile
                        res_b1 = cp.clip(r_gpu.astype(cp.float32) * 1.05, 0, 255).astype(cp.uint8)
                ...
                if (flag_histogram_stretch == 1 or flag_histogram_equalize2 == 1 or flag_histogram_phitheta == 1) :
                    ...
                    if flag_histogram_phitheta == 1 :
                        # OPT: vectorisé NumPy
                        trsf_r[1:] = np.clip((255.0 / (1.0 + np.exp(-val_phi * ((trsf_r[1:] - val_theta) / 32.0)))).astype(int), 0, 255)
...
def video_capture(image_sauve) :
    global image_traitee,start_video,nb_cap_video,nb_acq_video,labelInfo1,flag_cap_video,video_path,val_nb_capt_video,video,videoOut,echelle11,flag_filtrage_ON,\
           ...
           flag_camera_ok,previous_frame_number,frame_number,flag_TRKSAT,flag_birds
                    ...
                    if nb_cap_video <= val_nb_capt_video :
                        if flag_GO == True :
                            if frame_number > previous_frame_number :
                                ...
                                nb_cap_video += 1
                if flag_TRIGGER == 1 and (flag_TRKSAT == 1 or flag_birds ==1) :
                    if nb_cap_video <= val_nb_capt_video :
...
def load_video() :
    global flag_premier_demarrage,Video_Test,flag_image_mode,flag_image_video_loaded,flag_SAT_Image,Sat_Vid_Img,flag_SER_file,flag_16b,SER_depth
...
def commande_DETECT_STARS() :
    global flag_DETECT_STARS,flag_nouvelle_resolution,flag_stars_detect_first_start
    ...
    if choix_DETECT_STARS.get() == 0 :
        ...
        flag_DETECT_STARS = 1
        flag_nouvelle_resolution = True
    flag_stars_detect_first_start = True
...
def commande_reduce_variation_post_treatment() :
    global flag_reduce_variation_post_treatment,max_qual_PT,flag_BFREFPT,flag_BFREF_image_PT
...
def choix_HDR_Percentile_Value(event=None) :
    global HDR_Percentile_Value, echelle806

    HDR_Percentile_Value=echelle806.get()
...
def reset_general_FS():
    global val_FS,compteur_FS,Im1OK,Im2OK,Im3OK,Im4OK,Im5OK,stack_div,echelle20,choix_AANR,choix_AANRB,flag_first_sat_pass,nb_sat,\
           ...
           img1_3FNR2OKB,img2_3FNR2OKB,img3_3FNR2OKB,FNR2_First_StartB,choix_3FNR2B,FNRB_First_Start,FNR2B_First_Start,flag_BFREF_image,\
           ...
           choix_sub_img_ref,flag_capture_image_reference,flag_image_ref_sub,flag_image_reference_OK,flag_stars_detect_first_start,flag_16b,flag_SER_file,SER_depth
...
def choix_Mu(event=None) :
    global val_Mu, echelle182, Corr_GS, Corr_GS_gpu, flag_lin_gauss
    
    val_Mu = echelle82.get()
    if flag_lin_gauss == 2 or flag_lin_gauss == 3 :
        # OPT: vectorisé NumPy + mise à jour cache GPU
        _x = np.arange(255, dtype=np.float32)
        Corr_GS = np.exp(-0.5 * ((_x * 0.0392157 - 5 - val_Mu) / val_Ro) ** 2).astype(np.float32)
        Corr_GS_gpu = cp.asarray(Corr_GS)


def choix_Ro(event=None) :
    global val_Ro, echelle184, Corr_GS, Corr_GS_gpu, flag_lin_gauss
...
def choix_Var_CLL(event=None) :
    global val_MuCLL, val_RoCLL, val_AmpCLL, echelle200, echelle201, echelle202, Corr_CLL, Corr_CLL_gpu
    
    val_MuCLL = echelle200.get()
    val_RoCLL = echelle201.get()
    val_AmpCLL = echelle202.get()
    # OPT: calcul vectorisé NumPy + mise à jour du cache GPU
    _x = np.arange(256, dtype=np.float32)
    _Corr = np.exp(-0.5 * ((_x * 0.0392157 - val_MuCLL) / val_RoCLL) ** 2)
    _cll_f = _x / (1.0 + val_AmpCLL * _Corr)
    Corr_CLL = np.maximum.accumulate(np.clip(_cll_f, 0, 255).astype(np.uint8))
    Corr_CLL_gpu = cp.asarray(Corr_CLL, dtype=cp.uint8)  # mise à jour cache GPU
...
def commande_CONST() :
    global flag_CONST,flag_nouvelle_resolution,flag_stars_detect_first_start
...
CBAISAT.place(anchor="w",x=0, y=740)

labelDayLight = Label (cadre, text = " Daylight:")
labelDayLight.place (anchor="w",x=0, y=765)

CBDLB = Checkbutton(cadre,text="Birds", variable=choix_Birds,command=commande_Birds,onvalue = 1, offvalue = 0)
CBDLB.place(anchor="w",x=0, y=780)


# Track Meteor
CBTRIGGER = Checkbutton(cadre,text="Trigger", variable=choix_TRIGGER,command=commande_TRIGGER,onvalue = 1, offvalue = 0)
CBTRIGGER.place(anchor="w",x=0, y=815)
...
labelParam804.place(anchor="w", x=1840+delta_s,y=10)
echelle804 = Scale (cadre, from_ = 8, to = 16, command= choix_TH_16B, orient=VERTICAL, length = 70, width = 7, resolution = 0.5, label="",showvalue=1,tickinterval=None,sliderlength=10)
...
CBHDR.place(anchor="w",x=1830+delta_s, y=120)
RBHDR1 = Radiobutton(cadre,text="Mertens", variable=mode_HDR_select,command=HDR_Mertens,value=1)
RBHDR1.place(anchor="w",x=1830+delta_s, y=140)
RBHDR2 = Radiobutton(cadre,text="Mean", variable=mode_HDR_select,command=HDR_Mean,value=2)
RBHDR2.place(anchor="w",x=1830+delta_s, y=160)
RBHDR3 = Radiobutton(cadre,text="Inv Percent", variable=mode_HDR_select,command=HDR_Var_Percent,value=3)
RBHDR3.place(anchor="w",x=1830+delta_s, y=180)
RBHDR4 = Radiobutton(cadre,text="Percentile", variable=mode_HDR_select,command=HDR_Percentile,value=4)
RBHDR4.place(anchor="w",x=1830+delta_s, y=200)
echelle806 = Scale (cadre, from_ = 0, to = 100, command= choix_HDR_Percentile_Value, orient=VERTICAL, length = 80, width = 7, resolution = 10, label="",showvalue=1,tickinterval=None,sliderlength=10)
echelle806.set(HDR_Percentile_Value)
echelle806.place(anchor="c", x=1850+delta_s,y=255)

# Set camera read speed
labelCamSpeed = Label (cadre, text = "Cam Speed")
labelCamSpeed.place (anchor="w",x=1840+delta_s, y=315)
RBCS1 = Radiobutton(cadre,text="Fast", variable=cam_read_speed,command=choix_read_speed_fast,value=0)
RBCS1.place(anchor="w",x=1840+delta_s, y=335)
RBCS2 = Radiobutton(cadre,text="Slow", variable=cam_read_speed,command=choix_read_speed_slow,value=1)
RBCS2.place(anchor="w",x=1840+delta_s, y=355)

# Text in picture
CBTIP = Checkbutton(cadre,text="TIP", variable=choix_TIP,command=commande_TIP,onvalue = 1, offvalue = 0)
CBTIP.place(anchor="w",x=1840+delta_s, y=390)

# Cross
CBCR = Checkbutton(cadre,text="Cr", variable=choix_cross,command=commande_cross,onvalue = 1, offvalue = 0)
CBCR.place(anchor="w",x=1840+delta_s, y=420)

# Histogram
CBHST = Checkbutton(cadre,text="Hst", variable=choix_HST,command=commande_HST,onvalue = 1, offvalue = 0)
CBHST.place(anchor="w",x=1840+delta_s, y=445)

# Histogram
CBTRSF = Checkbutton(cadre,text="Trsf", variable=choix_TRSF,command=commande_TRSF,onvalue = 1, offvalue = 0)
CBTRSF.place(anchor="w",x=1840+delta_s, y=470)
...
CBTRGS.place(anchor="w",x=1840+delta_s, y=495)
...
CBAS = Checkbutton(cadre,text="Ampli Soft", variable=choix_AmpSoft,command=commande_AmpSoft,onvalue = 1, offvalue = 0)
CBAS.place(anchor="w",x=1440+delta_s, y=160)
echelle80 = Scale (cadre, from_ = 0, to = 10.0, command= choix_amplif, orient=HORIZONTAL, length = 150, width = 7, resolution = 0.1, label="",showvalue=1,tickinterval=1,sliderlength=20)
echelle80.set(val_ampl)
echelle80.place(anchor="w", x=1505+delta_s,y=160)

# Set Gamma value ZWO camera
labelParam204 = Label (cadre, text = "Gamma Cam")
labelParam204.place(anchor="w", x=1665+delta_s,y=160)
echelle204 = Scale (cadre, from_ = 0, to = 100, command= choix_ASI_GAMMA, orient=HORIZONTAL, length = 100, width = 7, resolution = 1, label="",showvalue=1,tickinterval=None,sliderlength=10)
echelle204.set(ASIGAMMA)
echelle204.place(anchor="c", x=1770+delta_s,y=155)
...
echelle84 = Scale (cadre, from_ = 0.2, to = 5.0, command= choix_Ro, orient=HORIZONTAL, length = 90, width = 7, resolution = 0.1, label="",showvalue=1,tickinterval=1,sliderlength=20)
...
echelle270 = Scale (cadre, from_ = 0.5, to = 10, command= choix_val_reduce_variation, orient=HORIZONTAL, length = 100, width = 7, resolution = 0.1, label="",showvalue=1,tickinterval=2,sliderlength=20)
...
echelle9 = Scale (cadre, from_ = 0.1, to = 8, command= choix_valeur_CLAHE, orient=HORIZONTAL, length = 100, width = 7, resolution = 0.1, label="",showvalue=1,tickinterval=1,sliderlength=20)
...
def init_camera() :
    ...
    if flag_ASI_OK == True :
        ...
        if flag_camera_ok == True :
            ...
            if "ZWO ASI1600MC" in cameras_found[0] or "ZWO ASI1600MM" in cameras_found[0] :
                res_cam_x = 4656
                res_cam_y = 3520
                cam_displ_x = int(1350*fact_s)
                cam_displ_y = int(1012*fact_s)
            ...
            if "ZWO ASI294MC" in cameras_found[0] or "ZWO ASI294MC Pro" in cameras_found[0] :
                res_cam_x = 4144
                res_cam_y = 2822
                cam_displ_x = int(1350*fact_s)
                cam_displ_y = int(1012*fact_s)
            ...
            if "ZWO ASI294MM" in cameras_found[0] or "ZWO ASI294MM Pro" in cameras_found[0] :
                res_cam_x = 8288
                res_cam_y = 5644
                cam_displ_x = int(1350*fact_s)
                cam_displ_y = int(1012*fact_s)
            ...
            if "ZWO ASI178MC" in cameras_found[0] or "ZWO ASI178MM" in cameras_found[0] or "ZWO ASI178MM Pro" in cameras_found[0] :
                res_cam_x = 3096
                res_cam_y = 2080
                cam_displ_x = int(1350*fact_s)
                cam_displ_y = int(1012*fact_s)
            ...
            if "ZWO ASI183MC"in cameras_found[0] or "ZWO ASI183MM" in cameras_found[0] or "ZWO ASI183MC Pro" in cameras_found[0] or "ZWO ASI183MM Pro" in cameras_found[0] :
                res_cam_x = 5496
                res_cam_y = 3672
                cam_displ_x = int(1350*fact_s)
                cam_displ_y = int(1012*fact_s)
            ...
            if "ZWO ASI485MC" in cameras_found[0] or "ZWO ASI585MC" in cameras_found[0] or "ZWO ASI585MM" in cameras_found[0] or "ZWO ASI678MC" in cameras_found[0] or "ZWO ASI678MM" in cameras_found[0] or "ZWO ASI715MC" in cameras_found[0] :
                res_cam_x = 3840
                res_cam_y = 2160
                cam_displ_x = int(1350*fact_s)
                cam_displ_y = int(760*fact_s)
            ...
            if "ZWO ASI290MM Mini" in cameras_found[0] or "ZWO ASI290MC" in cameras_found[0] or "ZWO ASI290MM" in cameras_found[0] or "ZWO ASI462MC" in cameras_found[0] or "ZWO ASI385MC" in cameras_found[0] :
                res_cam_x = 1936
                res_cam_y = 1096
                cam_displ_x = int(1350*fact_s)
                cam_displ_y = int(760*fact_s)
            ...
            if "ZWO ASI662MC" in cameras_found[0] or "ZWO ASI482MC" in cameras_found[0] :
                res_cam_x = 1920
                res_cam_y = 1080
                cam_displ_x = int(1350*fact_s)
                cam_displ_y = int(760*fact_s)
            ...
            if "ZWO ASI224MC" in cameras_found[0] :
                res_cam_x = 1304
                res_cam_y = 976
                cam_displ_x = int(1350*fact_s)
                cam_displ_y = int(1012*fact_s)
            ...
            if "ZWO ASI533MC" in cameras_found[0] or "ZWO ASI533MM" in cameras_found[0] or "ZWO ASI533MC Pro" in cameras_found[0] or "ZWO ASI533MM Pro" in cameras_found[0] :
                res_cam_x = 3008
                res_cam_y = 2260
                cam_displ_x = int(1012*fact_s)
                cam_displ_y = int(1012*fact_s)
            ...
            if "ZWO ASI676MC" in cameras_found[0] :
                res_cam_x = 3552
                res_cam_y = 3552
                cam_displ_x = int(1012*fact_s)
                cam_displ_y = int(1012*fact_s)
            ...
            if "ZWO ASI120MC" in cameras_found[0] or "ZWO ASI120MM" in cameras_found[0] :
                res_cam_x = 1280
                res_cam_y = 960
                cam_displ_x = int(1350*fact_s)
                cam_displ_y = int(760*fact_s)
