...
if my_os == "win32" :
    ...
    titre = "JetsonSky " +  JetsonSky_version + " Windows release - CUDA / OPENCV / CUPY / VPI / TorchVision - Copyright Alain PAILLOU 2018-2023"

if my_os == "linux" :
    Dev_system = "Jetson Linux"
    print("Linux system")
    titre = "JetsonSky " +  JetsonSky_version + " Linux release - CUDA / OPENCV / CUPY / VPI / TorchVision - Copyright Alain PAILLOU 2018-2023" 
...
class acquisition(Thread) :
    ...
    def run(self) :
        global type_debayer,camera,nb_erreur,image_brute,flag_acquisition_en_cours,flag_autorise_acquisition,Date_hour_image,image_camera,\
               flag_image_disponible,frame,flag_noir_blanc,flag_hold_picture,val_delai,flag_BCap,image_BG,flag_sub_dark,Master_Dark,image_camera_old,\
...
def refresh() :
    global video,flag_cap_video,camera,traitement,img_cam,cadre_image,rawCapture,image_affichee,image_brute,flag_image_disponible,flag_quitter,timeoutexp,flag_new_image,\
           ...
           val_gain, echelle2,val_exposition,echelle1,imggrey1,imggrey2,flag_FDIF,FDIF,flag_TRKSAT,correspondance,labelInfo2,Date_hour_image,image_camera,\
           calque_stars,calque_satellites,calque_TIP,flag_nouvelle_resolution,nb_sat,sat_x,sat_y,sat_old_x,sat_old_y,res_bb1,res_gg1,res_rr1,image_camera_old,\
           calque_direction_satellites,correspondance,vitesse,sat_id,sat_old_id,compteur_sat,old_sat,sat_speed,compteur_images,numero_image,nb_erreur,curFPS, fpsQueue,\
           labelInfo2,start_time_video,stop_time_video,time_exec_test,total_time,font,image_camera_old,image_camera,val_nb_capt_video,res_cam_x,res_cam_y,flag_premier_demarrage,Video_Test
    ...
    if flag_camera_ok == True :
        ...
        if ((flag_image_disponible == True and flag_new_image == True) or flag_HDR == True) and (image_camera > numero_image):
            ...
            if flag_stop_acquisition == False and flag_filtre_work == False :
                ...
                labelInfo2.config(text = str(time_exec_test) + " ms   FPS : " + str(int(curFPS*10)/10) + "    ")
        ...
        if (video.isOpened()):
            ...
            if ret == True :
                ...
                if video_debayer != 0 :
                    ...
                    image_brute=cv2.cvtColor(image_brute, cv2.COLOR_BGR2RGB)
                res_bb1,res_gg1,res_rr1=cv2.split(image_brute)
                flag_image_disponible = True   
                application_filtrage(res_bb1,res_gg1,res_rr1)
                total_stop = cv2.getTickCount()
...
def satellites_tracking ():
    global imggrey1,imggrey2,image_traitee,calque_satellites,flag_sat_exist,sat_x,sat_y,sat_s,sat_old_x,sat_old_y,sat_cntdwn,nb_sat,sat_id,compteur_sat, \
           flag_first_sat_pass,nb_trace_sat,sat_speed,calque_direction_satellites
...
def application_filtrage(res_b1,res_g1,res_r1) :
    global image_traitee1,image_traitee2,image_traitee3,image_traitee4,image_traitee5,compteur_FS,Im1OK,Im2OK,Im3OK,compteur_images,numero_image,\
           Im4OK,Im5OK,flag_hold_picture,image_brute,img_brut_tmp,val_denoise,val_denoise_KNN,val_histo_min,val_histo_max,image_camera,image_camera_old, \
           ...
           flag_dyn_AADP,Corr_GS,calque_TIP,azimut,hauteur,flag_type_clahe,val_ghost_reducer,res_b2,res_g2,res_r2,time_exec_test,flag_HDR
    ...
    if flag_filtrage_ON == True :
        ...
        if flag_noir_blanc == 1 or flag_colour_camera == False :
            ...
            # Contrast PyTorch
            if flag_contrast_PYTORCH ==1 :
                image_PIL = PIL.Image.fromarray(imag_brute)
                image_PIL_contrat = F.adjust_contrast(image_PIL,val_contrast_PYTORCH)
                imag_brute = np.asarray(image_PIL_contrat)
        ...
        if flag_noir_blanc == 0 and flag_colour_camera == True:
            ...
            if flag_camera_ok == False :
                if type_flip == "vertical" :
                    res_b1 = cv2.flip(res_b1,0)
                    res_g1 = cv2.flip(res_g1,0)
                    res_r1 = cv2.flip(res_r1,0)
                if type_flip == "horizontal" :
                    res_b1 = cv2.flip(res_b1,1)
                    res_g1 = cv2.flip(res_g1,1)
                    res_r1 = cv2.flip(res_r1,1)
                if type_flip == "both" :
                    res_b1 = cv2.flip(res_b1,-1)
                    res_g1 = cv2.flip(res_g1,-1)
                    res_r1 = cv2.flip(res_r1,-1)
            ...
            if flag_2DConv == 1 or ((flag_histogram_stretch == 1 or (flag_histogram_equalize2 == 1 and type_gamma == "cuda") or flag_histogram_phitheta == 1) and flag_front == True) or\
               (flag_AmpSoft == 1  and (flag_lin_gauss == 1 or flag_lin_gauss == 2)) or flag_denoise_Paillou == 1 or flag_denoise_soft == 1 or flag_denoise_KNN == 1 or\
               val_reds != 100 or val_greens != 100 or val_blues != 100 :
            ...
            if flag_contrast_PYTORCH ==1 and flag_torchvision == True :
                image_PIL = PIL.Image.fromarray(cv2.merge((res_b1,res_g1,res_r1)))
                image_PIL_contrat = F.adjust_contrast(image_PIL,val_contrast_PYTORCH)
                numpy_img = np.asarray(image_PIL_contrat)
                res_b1,res_g1,res_r1=cv2.split(numpy_img)
            ...
            if flag_SAT == True :
                ...
                if flag_torchvision == True :
                    img_sat = F.adjust_saturation(img_base,val_SAT)
                    res_b1,res_g1,res_r1 = cv2.split(np.asarray(img_sat))
                else :
...
def video_capture(image_sauve) :
    global image_traitee,start_video,nb_cap_video,nb_acq_video,labelInfo1,flag_cap_video,video_path,val_nb_capt_video,video,videoOut,\
           echelle11,flag_pause_video,timer1,val_deltat,FDIF,flag_FDIF,val_nb_capt_video
    if flag_camera_ok == True :
        if nb_cap_video == 1 :
            if flag_HQ == 0 :
                if flag_filter_wheel == True:
            ...
            if flag_HQ == 1 :
                ...
                if image_traitee.ndim == 3 :
                    ...
                    if Dev_system == "PC Windows" :
                        ...
                        gst_pipe = "appsrc ! video/x-raw,format=BGR,width=" + str(width) + ",height=" + str(height) + ",framerate=25/1 ! queue ! videoconvert ! video/x-raw,format=BGRx ! nvvidconv ! nvv4l2h264enc qp-range=0,1:0,1:0,1 profile=4 control-rate=0 ! h264parse ! qtmux ! filesink location=" + path_video
                        video = cv2.VideoWriter(gst_pipe, cv2.CAP_GSTREAMER, 0, float(25), (width, height), isColor = (image_traitee.ndim > 1))
                else :
                    height,width = image_sauve.shape
                    if Dev_system == "PC Windows" :
                        gst_pipe = "appsrc ! video/x-raw,format=GRAY8,width=" + str(width) + ",height=" + str(height) + ",framerate=25/1 ! queue ! videoconvert ! video/x-raw,format=GRAY8 ! nvh264enc ! h264parse ! qtmux ! filesink location=" + path_video
                        video = cv2.VideoWriter(gst_pipe, cv2.CAP_GSTREAMER, 0, float(25), (width, height), isColor = (image_traitee.ndim > 1))
                    else :
...
echelle70 = Scale (cadre, from_ = 0, to = 15, command= choix_val_SAT, orient=VERTICAL, length = 100, width = 7, resolution = 0.1, label="",showvalue=1,tickinterval=None,sliderlength=10)
...
if flag_camera_ok == True :
    ...
    labelFW.place (anchor="w",x=5, y=560)
...
CBBL.place(anchor="w",x=1610, y=400)
...
CBTVS.place(anchor="w",x=1450, y=430)
...
echelle230.place(anchor="w", x=1560,y=430)
...
CBEPF.place(anchor="w",x=1450, y=470)
...
echelle130.place(anchor="w", x=1690,y=463)
...
CBEPF.place(anchor="w",x=1450, y=500)
...
echelle30.place(anchor="w", x=1510,y=500)
...
CBDS.place(anchor="w",x=1645, y=500)
...
echelle4.place(anchor="w", x=1710,y=500)
...
CBGR.place(anchor="w",x=1450, y=540)
...
echelle60.place(anchor="w", x=1600,y=540)

# Choix Parametre Atenuation Gradient Removal
labelParam61 = Label (cadre, text = "At")
labelParam61.place(anchor="e", x=1735,y=540)
...
echelle61.place(anchor="w", x=1740,y=540)
...
CBIN.place(anchor="w",x=1450, y=580)
...
CBHE2.place(anchor="w",x=1520, y=580)
...
echelle16.place(anchor="w", x=1580,y=580)
...
echelle5.place(anchor="w", x=1580,y=620)

labelParam6 = Label (cadre, text = "Max") # choix valeur histogramme strech maximum
labelParam6.place(anchor="w", x=1700,y=620)
...
echelle6.place(anchor="w", x=1720,y=620)
...
echelle12.place(anchor="w", x=1580,y=660)

labelParam13 = Label (cadre, text = "Dec") # choix valeur histogramme Signoide param 2
labelParam13.place(anchor="w", x=1700,y=660)
...
echelle13.place(anchor="w", x=1720,y=660)
...
CBCC.place(anchor="w",x=1450, y=700)

labelParam9 = Label (cadre, text = "Clip") # choix valeur contrate CLAHE
labelParam9.place(anchor="w", x=1555,y=700)
...
echelle9.place(anchor="w", x=1580,y=700)
labelParam109 = Label (cadre, text = "Grid") # choix valeur contrate CLAHE
labelParam109.place(anchor="w", x=1700,y=700)
...
echelle109.place(anchor="w", x=1720,y=700)
...
CBCPT.place(anchor="w",x=1450, y=740)

labelParam99 = Label (cadre, text = "Value") # choix valeur contrate CLAHE
labelParam99.place(anchor="w", x=1555,y=740)
...
echelle99.place(anchor="w", x=1580,y=740)
...
if flag_camera_ok == False :
    ...
    Button12.place(anchor="w", x=0,y=830)
