...
def refresh() :
    global video,flag_cap_video,camera,traitement,cadre_image,image_brute,flag_image_disponible,flag_quitter,timeoutexp,flag_new_image,curFPS,fpsQueue,image_brut_read,delta_zx,delta_zy,flag_new_stab_window,\
           ...
           delta_RX,delta_RY,delta_BX,delta_BY,track,track_crater_history,track_sat_history,track_sat,track_crater,key_pressed
...
def Template_tracking(image,dim) :
    global flag_STAB,flag_Template,Template,gsrc,gtmpl,gresult,matcher,delta_tx,delta_ty,start_point,end_point,flag_new_stab_window,key_pressed
    ...
    if flag_Template == False :
        if res_cam_x > 1500 :
            ...
            rs = res_cam_y // 2 - res_cam_y // 3 + delta_ty
            re = res_cam_y // 2 + res_cam_y // 3 + delta_ty
            cs = res_cam_x // 2 - res_cam_x // 3 + delta_tx
            ce = res_cam_x // 2 + res_cam_x // 3 + delta_tx
            if cs < 30 or ce > (res_cam_x - 30) :
                delta_tx = old_tx
                cs = res_cam_x // 2 - res_cam_x // 3 + delta_tx
                ce = res_cam_x // 2 + res_cam_x // 3 + delta_tx
...
def commande_STAB() :
    global flag_STAB, flag_Template,delta_tx,delta_ty
    delta_tx = 0
    delta_ty = 0
    if choix_STAB.get() == 0 :
        flag_STAB = False
...
if flag_camera_ok == True :
    ...
    # Image format
    labelSR = Label (cadre, text = "Im Ratio :")
