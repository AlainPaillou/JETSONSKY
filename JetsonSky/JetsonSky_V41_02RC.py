...
try :
    ...
    if torch.cuda.is_available():
        ...
        print("Pytorch : GPU is available and being used")
    else:
        device = torch.device("cpu")
        print("Pytorch : GPU is not available, using CPU instead")
...
choix_DETECT_STARS.set(0) # Initialisation Demo OFF
...
choix_VHD = IntVar ()
choix_VHD.set(0) # Initialisation AADF Very Hifh Dynamic

choix_TRKSAT = IntVar()
choix_TRKSAT.set(0) # Initialisation Remove background OFF

choix_TRIGGER = IntVar()
choix_TRIGGER.set(0) # Initialisation Remove background OFF
...
def init_camera() :
    ...
    if flag_camera_ok == True :
        ...
        if cameras_found[0] == "ZWO ASI485MC" or cameras_found[0] == "ZWO ASI585MC" or cameras_found[0] == "ZWO ASI678MC" or cameras_found[0] == "ZWO ASI678MM" :
            ...
            RES_X_BIN1_1_1 = [2160,1822,1620,1350,1012,864,680,540,336]
            RES_Y_BIN1_1_1 = [2160,1822,1620,1350,1012,864,680,540,336]
...
#            RES_X_BIN2_16_9 = [1920,1544,1280,960,800,640,512,400]
#            RES_Y_BIN2_16_9 = [1080,870,720,540,450,360,288,225]
...
def refresh() :
    global video,flag_cap_video,camera,traitement,cadre_image,image_brute_base,flag_image_disponible,flag_quitter,timeoutexp,flag_new_image,curFPS,fpsQueue,image_brut_read,\
           ...
           val_gain, echelle2,val_exposition,echelle1,imggrey1,imggrey2,flag_DETECT_STARS,flag_TRKSAT,labelInfo2,Date_hour_image,image_camera,val_nb_capt_video,flag_image_mode,\
           calque_stars,calque_satellites,calque_TIP,flag_nouvelle_resolution,nb_sat,sat_x,sat_y,res_bb1,res_gg1,res_rr1,image_camera_old,s1,flag_TRIGGER,\
           ...
           image_camera,res_cam_x,res_cam_y,flag_premier_demarrage,Video_Test,TTQueue,curTT,max_sat,video_frame_number,video_frame_position
            ...
            if ((flag_image_disponible == True and flag_new_image == True) or flag_HDR == True) and (image_camera > numero_image):
                if flag_HDR == True :
                    try :
                        ...
                        if flag_acq_rapide == "Fast" :
                            exposition = val_exposition // 5
                        else :
                            exposition = val_exposition*1000 // 5
                        camera.set_control_value(asi.ASI_EXPOSURE, exposition)
...
#                        alignMTB.process(img_list, img_list)
                ...
                if flag_stop_acquisition == False and flag_filtre_work == False :
                    ...
                    if flag_DETECT_STARS == 1 and flag_noir_blanc == 0 and flag_colour_camera == True :
                        stars_detection()
                    if flag_TRKSAT == 1 and nb_sat >= 0 and flag_noir_blanc == 0 and flag_colour_camera == True :
                    ...
                    if flag_TRGS == 1:
                        ...
                        for x in range(1,255) :
                            transform.line((((x-1)*3,SY-Corr_GS[x-1]*256),(x*3,SY-Corr_GS[x]*256)),fill="blue",width=4)
                        transform.line(((256*3,SY),(256*3,SY-256*2)),fill="red",width=3)
                        transform.line(((1,SY-256*2),(256*3,SY-256*2)),fill="red",width=3)
            ...
            if flag_image_video_loaded == True :
                ...
                if flag_TRKSAT == 1 and flag_noir_blanc == 0 and flag_colour_camera == True and flag_image_mode == False :
                    satellites_tracking()
                flag_sat_detected = False
                if flag_DETECT_STARS == 1 and flag_noir_blanc == 0 and flag_colour_camera == True and flag_image_mode == False :
                    stars_detection()
                if flag_TRKSAT == 1  and nb_sat >= 0 and flag_noir_blanc == 0 and flag_colour_camera == True and flag_image_mode == False :
                    font = cv2.FONT_HERSHEY_SIMPLEX
                    size = 0.5
                    for i in range(nb_sat+1):
                        centercircle = (sat_x[i],sat_y[i])
                ...
                if (res_cam_x > int(cam_displ_x*fact_s) or res_cam_y > int(cam_displ_y*fact_s)) and flag_full_res == 0 :
                    image_traitee_resize = cv2.resize(image_traitee,(int(cam_displ_x*fact_s),int(cam_displ_y*fact_s)),interpolation = cv2.INTER_AREA)
                    cadre_image.im=PIL.Image.fromarray(image_traitee_resize)
                if (res_cam_x > int(1350*fact_s) or res_cam_y > int(1012*fact_s)) and flag_full_res == 1 :
                    rs = (res_cam_y - int(1012*fact_s)) // 2 - 1
                    re = (res_cam_y + int(1012*fact_s)) // 2 + 1
...
def application_filtrage(res_b1,res_g1,res_r1) :
    global compteur_FS,Im1OK,Im2OK,Im3OK,compteur_images,numero_image,b1_sm, b2_sm, b3_sm, b4_sm, b5_sm, g1_sm, g2_sm, g3_sm, g4_sm, g5_sm, r1_sm, r2_sm, r3_sm, r4_sm, r5_sm,\
           ...
           flag_dyn_AADP,Corr_GS,azimut,hauteur,val_ghost_reducer,res_b2,res_g2,res_r2,time_exec_test,flag_HDR,val_sharpen,val_sharpen2
    ...
    with s1 :     
        if flag_filtrage_ON == True :
            ...
            if flag_noir_blanc == 1 or flag_colour_camera == False :
                ...
                if flag_denoise_stacking_Paillou == True and flag_hold_picture == 0 and flag_image_mode == False :
            ...
            if flag_noir_blanc == 0 and flag_colour_camera == True:
                ...
                if flag_contrast_PYTORCH ==1 and flag_torchvision == True :
                    ...
                    correction_contrast = cp.asarray(Corr_cont)

                    Colour_contrast_GPU((nb_blocksX,nb_blocksY),(nb_ThreadsX,nb_ThreadsY),(r_gpu, g_gpu, b_gpu, res_r1, res_g1, res_b1, np.int_(width), np.int_(height), \
                        np.float32(val_ampl), correction_contrast))
                ...
                if flag_SAT == True :
                    ...
                    coul_gauss_b,coul_gauss_g,coul_gauss_r = gaussianblur_colour(res_r1,res_g1,res_b1,13)

                    colour_base_gauss = cv2.merge((coul_gauss_r.get(),coul_gauss_g.get(),coul_gauss_b.get()))
         
                    img_base_gauss=PIL.Image.fromarray(colour_base_gauss) 
                    if flag_torchvision == True :
                        img_sat_gauss = F.adjust_saturation(img_base_gauss,val_SAT)
                        if ImageNeg == 1 :
                            img_sat_gauss = F.adjust_hue(img_sat_gauss,0.5)
                    else :
                        converter = ImageEnhance.Color(img_base_gauss)
                        img_sat_gauss = converter.enhance(val_SAT)

                    coul_ext_r,coul_ext_g,coul_ext_b=cupy_RGBImage_2_cupy_separateRGB(cp.array(img_sat_gauss))

                    r_gpu = res_r1
                    g_gpu = res_g1
                    b_gpu = res_b1

                    Saturation_Combine_Colour((nb_blocksX,nb_blocksY),(nb_ThreadsX,nb_ThreadsY),(r_gpu, g_gpu, b_gpu, res_r1, res_g1, res_b1, coul_ext_r, coul_ext_g, coul_ext_b, np.int_(width), np.int_(height)))
...
def video_capture(image_sauve) :
    global image_traitee,start_video,nb_cap_video,nb_acq_video,labelInfo1,flag_cap_video,video_path,val_nb_capt_video,video,videoOut,echelle11,\
...
def load_video() :
    ...
    if Video_name != "" :
        ...
        flag_premier_demarrage = True
        if Video_Test.lower().endswith(('.mp4', '.avi', '.mov')) :
            flag_image_mode = False
...
def choix_valeur_contrast_PYTORCH(event=None) :
    global val_contrast_PYTORCH,echelle99,Corr_cont
    val_contrast_PYTORCH=echelle99.get()
    for x in range(0,255) :
        Corr_cont[x] = (1-math.exp(-1*x/val_contrast_PYTORCH)+beta)/(1+beta)
...
def commande_VHD() :
    global choix_VHD, flag_AADF_VHD
    if choix_VHD.get() == 0 :
        flag_AADF_VHD = False
    else :
        flag_AADF_VHD = True
...
labelParam200 = Label (cadre, text = "Gamma Cam")
labelParam200.place(anchor="w", x=1840+delta_s,y=90)
echelle200 = Scale (cadre, from_ = 0, to = 100, command= choix_ASI_GAMMA, orient=VERTICAL, length = 130, width = 7, resolution = 1, label="",showvalue=1,tickinterval=None,sliderlength=10)
echelle200.set(ASIGAMMA)
echelle200.place(anchor="c", x=1840+delta_s,y=170)
...
CBTRSF.place(anchor="w",x=1840+delta_s, y=195+240)
...
CBOP.place(anchor="w",x=1730+delta_s, y=245)
...
labelParam14.place(anchor="w", x=1450+delta_s,y=320)
...
echelle14.place(anchor="w", x=1485+delta_s,y=320)

# Camera Blue balance
labelParam15 = Label (cadre, text = "CBlue") # choix balance bleue
labelParam15.place(anchor="w", x=1645+delta_s,y=320)
...
echelle15.place(anchor="w", x=1680+delta_s,y=320)

# Software Red balance
labelParam100 = Label (cadre, text = "R") # choix balance rouge
labelParam100.place(anchor="w", x=1440+delta_s,y=360)
...
echelle100.place(anchor="w", x=1440+delta_s+15,y=360)

# Software Green balance
labelParam101 = Label (cadre, text = "G") # choix balance rouge
labelParam101.place(anchor="w", x=1570+delta_s,y=360)
...
echelle101.place(anchor="w", x=1570+delta_s+15,y=360)

# Software Blue balance
labelParam102 = Label (cadre, text = "B") # choix balance bleue
labelParam102.place(anchor="w", x=1700+delta_s,y=360)
...
echelle102.place(anchor="w", x=1700+delta_s+15,y=360)
...
CBSS1.place(anchor="w",x=1450+delta_s, y=405)
...
echelle152.place(anchor="w", x=1560+delta_s,y=405)
...
echelle153.place(anchor="w", x=1690+delta_s,y=405)
...
CBSS2.place(anchor="w",x=1450+delta_s, y=435)
...
echelle154.place(anchor="w", x=1560+delta_s,y=435)
...
echelle155.place(anchor="w", x=1690+delta_s,y=435)
...
CBTVS.place(anchor="w",x=1450+delta_s, y=465)
...
echelle230.place(anchor="w", x=1560+delta_s,y=465)
...
CBEPF.place(anchor="w",x=1450+delta_s, y=510)
...
CBEPFS.place(anchor="w",x=1520+delta_s, y=510)
# AADF Mode
RBAADP1 = Radiobutton(cadre,text="H", variable=choix_dyn_AADP,command=choix_dyn_high,value=1)
RBAADP1.place(anchor="w",x=1565+delta_s, y=510)
RBAADP2 = Radiobutton(cadre,text="L", variable=choix_dyn_AADP,command=choix_dyn_low,value=2)
RBAADP2.place(anchor="w",x=1595+delta_s, y=510)

# AADF Very High Dynamic
CBVHD = Checkbutton(cadre,text="VHD", variable=choix_VHD,command=commande_VHD,onvalue = 1, offvalue = 0)
CBVHD.place(anchor="w",x=1625+delta_s, y=510)

# AADF ghost reducer
CBGR = Checkbutton(cadre,text="GR", variable=choix_ghost_reducer,command=commande_ghost_reducer,onvalue = 1, offvalue = 0)
CBGR.place(anchor="w",x=1686+delta_s, y=510)
...
echelle130.place(anchor="w", x=1720+delta_s,y=503)
...
CBEPF.place(anchor="w",x=1450+delta_s, y=540)
...
echelle30.place(anchor="w", x=1510+delta_s,y=540)
...
CBDS.place(anchor="w",x=1645+delta_s, y=540)
...
echelle4.place(anchor="w", x=1710+delta_s,y=540)
...
CBGR.place(anchor="w",x=1450+delta_s, y=580)
...
echelle60.place(anchor="w", x=1600+delta_s,y=580)

# Choix Parametre Atenuation Gradient Removal
labelParam61 = Label (cadre, text = "At")
labelParam61.place(anchor="e", x=1735+delta_s,y=580)
...
echelle61.place(anchor="w", x=1740+delta_s,y=580)
...
CBIN.place(anchor="w",x=1450+delta_s, y=620)
...
CBHE2.place(anchor="w",x=1520+delta_s, y=620)
...
echelle16.place(anchor="w", x=1580+delta_s,y=620)
...
CBHS.place(anchor="w",x=1450+delta_s, y=660)

labelParam5 = Label (cadre, text = "Min") # choix valeur histogramme strech minimum
labelParam5.place(anchor="w", x=1555+delta_s,y=660)
...
echelle5.place(anchor="w", x=1580+delta_s,y=660)

labelParam6 = Label (cadre, text = "Max") # choix valeur histogramme strech maximum
labelParam6.place(anchor="w", x=1700+delta_s,y=660)
...
echelle6.place(anchor="w", x=1720+delta_s,y=660)
...
CBHPT.place(anchor="w",x=1450+delta_s, y=700)

labelParam12 = Label (cadre, text = "Pnt") # choix valeur histogramme Signoide param 1
labelParam12.place(anchor="w", x=1555+delta_s,y=700)
...
echelle12.place(anchor="w", x=1580+delta_s,y=700)

labelParam13 = Label (cadre, text = "Dec") # choix valeur histogramme Signoide param 2
labelParam13.place(anchor="w", x=1700+delta_s,y=700)
...
echelle13.place(anchor="w", x=1720+delta_s,y=700)
...
CBCC.place(anchor="w",x=1450+delta_s, y=740)

labelParam9 = Label (cadre, text = "Clip") # choix valeur contrate CLAHE
labelParam9.place(anchor="w", x=1555+delta_s,y=740)
...
echelle9.place(anchor="w", x=1580+delta_s,y=740)
labelParam109 = Label (cadre, text = "Grid") # choix valeur contrate CLAHE
labelParam109.place(anchor="w", x=1700+delta_s,y=740)
...
echelle109.place(anchor="w", x=1720+delta_s,y=740)
...
CBCPT.place(anchor="w",x=1450+delta_s, y=780)

labelParam99 = Label (cadre, text = "Value") # choix valeur contrate CLAHE
labelParam99.place(anchor="w", x=1555+delta_s,y=780)
echelle99 = Scale (cadre, from_ = 5, to = 100, command= choix_valeur_contrast_PYTORCH, orient=HORIZONTAL, length = 240, width = 7, resolution = 5, label="",showvalue=1,tickinterval=0.5,sliderlength=20)
echelle99.set(val_contrast_PYTORCH)
echelle99.place(anchor="w", x=1580+delta_s,y=780)
...
CBHQC.place(anchor="w",x=1450+delta_s, y=990)
...
echelle8.place(anchor="w", x=1570+delta_s,y=863)
...
echelle11.place(anchor="w", x=1570+delta_s,y=910)

labelParam65 = Label (cadre, text = "Delta T") # choix valeur contrate CLAHE
labelParam65.place(anchor="w", x=1535+delta_s,y=955)
...
echelle65.place(anchor="w",x=1570+delta_s,y=955)

labelInfo1 = Label (cadre, text = text_info1) # label info n°1
labelInfo1.place(anchor="w", x=1550+delta_s,y=990)
...
labelInfoDark.place(anchor="w", x=1750+delta_s,y=820)
...
CBAD.place(anchor="w",x=1685+delta_s, y=823)
...
echelle10.place(anchor="w", x=1575+delta_s,y=823)
...
Button (fenetre_principale, text = "Quit", command = quitter,padx=10,pady=5).place(x=1700+delta_s,y=1020, anchor="w")
