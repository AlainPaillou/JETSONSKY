...
#        Images/videos acquisition & treatment software for ZWO ASI178MC, ASI178MM, ASI485MC & ASI 585MC camera            #
#                                                                     Copyright Alain Paillou 2018-2022                                                                    #

############################################################################################################
############################################################################################################

# Choose the right directories for video & image  PC DD/SSD 
# Choose the right directory for ZWO libray
...
if Dev_system == "Jetson Linux" :
    ...
    # Path librairies ZWO Windows
    sys.path.insert(0, "D:\Soft\JetsonSky\Lib")
    env_filename_camera = "D:\Soft\JetsonSky\Lib\ASICamera2.dll"
    env_filename_efw = "D:\Soft\JetsonSky\Lib\EFW_filter.dll"
...
# Programm Title
titre = "JetsonSky V12.00release - CUDA / OPENCV - Copyright Alain PAILLOU 2018-2022"

# Setting the Images & videos quality
flag_HQ = 0 # si 1 : HQ RAW format    si 0 :  LQ compressed format
...
sat_cntdwn  = np.zeros(1000,dtype=int)
...
choix_autoexposure.set(0) # Initialisation AUTOEXPOSURE

choix_autogain = IntVar ()
choix_autogain.set(0) # Initialisation AUTOGAIN
...
choix_SAT = IntVar ()
choix_SAT.set(0) # réglage saturation couleurs
...
//      mul_factor = __expf(-0.5*__powf((grey_gpu[index]*0.03921568-5-val_Mu)/val_Ro,2));
      ...
      dest_g[index] = (int)(min(max(int(img_g[index] + delta*mul_factor*val_ampl), 0), 255));
      dest_b[index] = (int)(min(max(int(img_b[index] + delta*mul_factor*val_ampl), 0), 255));
  }
}
""")
...
def init_camera() :
    ...
    if cameras_found[0] == "ZWO ASI178MC" or cameras_found[0] == "ZWO ASI178MM" :
        res_x_max = 3096
        res_y_max = 2080
        res_cam_x = 3096
        res_cam_y = 2080
    ...
    if cameras_found[0] == "ZWO ASI485MC" or cameras_found[0] == "ZWO ASI585MC" :
        res_x_max = 3840
        res_y_max = 2160
        res_cam_x = 3840
        res_cam_y = 2160
...
def init_efw():
    ...
    if num_efw == 0:
        ...
        filter_wheel.open()
#        fw_position = filter_wheel.get_position()
        fw_position = 0
...
class acquisition(Thread) :
    ...
    def run(self) :
        global type_debayer,camera,nb_erreur,image_brute,flag_acquisition_en_cours,flag_autorise_acquisition,Date_hour_image,\
               ...
               flag_nouvelle_resolution,calque_stars,calque_satellites,calque_TIP
        while flag_autorise_acquisition == True :
            if flag_stop_acquisition == False and flag_filtre_work == False :
                if flag_hold_picture == 0 :
                    ...
                    flag_image_disponible = False
                    try :
                        image_tmp=camera.capture_video_frame(filename=None)
                        ...
                        nb_erreur += 1
                        print("Acquisition error : ",nb_erreur)
                        time.sleep(0.01)
                else :
                    time.sleep(0.01)
            flag_acquisition_en_cours = False
            time.sleep(val_delai)
...
def refresh() :
    global camera,traitement, img_cam,cadre_image,rawCapture,image_affichee,image_brut,flag_image_disponible,flag_quitter,\
           ...
           val_gain, echelle2,val_exposition,echelle1,imggrey1,imggrey2,flag_FDIF,FDIF,flag_TRKSAT,\
           calque_stars,calque_satellites,calque_TIP,flag_nouvelle_resolution,nb_sat,sat_x,sat_y,sat_old_x,sat_old_y 
    ...
    if flag_image_disponible == True :
        if flag_stop_acquisition == False :
            if flag_nouvelle_resolution == True and flag_noir_blanc == 0 and flag_colour_camera == True :
                calque_stars = image_brute
                calque_stars[:,:,0] = 0
                calque_stars[:,:,1] = 0
                calque_stars[:,:,2] = 0                            
                calque_satellites = calque_stars
                flag_nouvelle_resolution = False
...
def satellites_tracking ():
    global imggrey1,imggrey2,image_traitee,calque_satellites,flag_sat_exist,sat_x,sat_y,sat_s,sat_old_x,sat_old_y,sat_cntdwn,nb_sat
    ...
    try :
        ...
        diff[0:50,0:750] = 0
        imggrey1 = imggrey2
        seuilb = np.percentile(diff, 99) + 25
...
def application_filtrage() :
    global image_traitee1,image_traitee2,image_traitee3,image_traitee4,image_traitee5,compteur_FS,Im1OK,Im2OK,Im3OK,\
           ...
           flag_dyn_AADP,Corr_GS,calque_TIP,azimut,hauteur
...
#            image_brut_CV = np.uint8(imag_brute)
            ...
            if flag_NB_estime == 1 :
                ...
                res_r = np.empty_like(imag_brute)
            ...
            if (val_FS > 1 and flag_hold_picture == 0) and Dev_system == "PC Windows" :
                ...
                if compteur_FS > val_FS :
                    compteur_FS = 1
                if compteur_FS == 1 :
                    image_traitee1 = imag_brute
                    Im1OK = True
                if compteur_FS == 2 :
                    image_traitee2 = imag_brute
                    Im2OK = True
                if compteur_FS == 3 :
                    image_traitee3 = imag_brute
                    Im3OK = True
                if compteur_FS == 4 :
                    image_traitee4 = imag_brute
                    Im4OK = True
                if compteur_FS == 5 :
                    image_traitee5 = imag_brute
                    Im5OK = True
        ...
        if flag_noir_blanc == 0 and flag_colour_camera == True:
            ...
            res_b1,res_g1,res_r1=cv2.split(imag_brute)
            ...
            if flag_denoise_stacking_Paillou == True and flag_hold_picture == 0 :
                ...
                if Im2fsdnOK == True :
                    ...
                    imag_brute = cv2.merge((res_b1,res_g1,res_r1))
                    old_image = imag_brute

                                
            if flag_AmpSoft == 1 and flag_lin_gauss == 3 :
                imagegrey = cv2.cvtColor(imag_brute, cv2.COLOR_BGR2GRAY)
            ...
            if flag_GR == True :
                if grad_vignet == 1 :
                    ...
                    res_b1 = np.uint8(corr_b)
                    res_g1 = np.uint8(corr_g)
                    res_r1 = np.uint8(corr_r)                
                    imag_brute = cv2.merge((res_b1,res_g1,res_r1))

             # Image negative
            if ImageNeg == 1 :
                res_b1 = cv2.bitwise_not(res_b1)
                res_g1 = cv2.bitwise_not(res_g1)
                res_r1 = cv2.bitwise_not(res_r1)
            ...
            if flag_SAT == True :
                ...
                res_b1,res_g1,res_r1 = cv2.split(image_saturee)
...
#        labelInfo1.config(text = str(time_exec_test) + " ms                 ")
    ...
    if flag_TIP == True :
        ...
        if width < 600 :
            size = 0.3
            pos = 15
        calque_TIP =np.empty_like(image_traitee)
...
def video_capture() :
    ...
    if nb_cap_video == 1 :
        ...
        if image_traitee.ndim == 3 :
            height,width,layers = image_traitee.shape
            if flag_HQ == 0 :
                video = cv2.VideoWriter(os.path.join(video_path,nom_video), fourcc, 25, (width, height), isColor = True) # compressed video
            else :
                video = cv2.VideoWriter(os.path.join(video_path,nom_video), fourcc, 25, (width, height), isColor=True) # video RAW
        else :
            height,width = image_traitee.shape
            if flag_HQ == 0 :
                video = cv2.VideoWriter(os.path.join(video_path,nom_video), fourcc, 25, (width, height), isColor = False) # compressed video
            else :
...
def mode_acq_lente() :
    ...
    exposition = val_exposition * 1000
    camera.set_control_value(asi.ASI_EXPOSURE, exposition)
    timeout = (camera.get_control_value(asi.ASI_EXPOSURE)[0] / 1000) * 2 + 1000
    camera.default_timeout = timeout
    camera.set_control_value(asi.ASI_HIGH_SPEED_MODE,0)
    time.sleep(0.1)
    flag_stop_acquisition=False
...
def valeur_exposition (event=None) :
    global timeout_val,flag_acq_rapide,camera,val_exposition,echelle1,val_resolution,flag_stop_acquisition,exposition,labelInfoDark,flag_sub_dark,dispo_dark,choix_sub_dark
    flag_sub_dark = False
    dispo_dark = 'Dark NON dispo '
    choix_sub_dark.set(0)
    labelInfoDark = Label (cadre, text = dispo_dark)
...
def choix_BIN1(event=None) :
    global val_FS,camera,val_resolution,echelle3,flag_acquisition_en_cours,flag_stop_acquisition,mode_BIN,labelInfoDark,flag_sub_dark,dispo_dark,choix_sub_dark,flag_nouvelle_resolution,flag_TIP,choix_TIP
    reset_general_FS()
    reset_FS()
    flag_TIP = 0
    choix_TIP.set(0)
...
def choix_resolution_camera(event=None) :
    global val_delai,val_FS,camera,traitement,val_resolution,res_cam_x,res_cam_y, img_cam,rawCapture,res_x_max,res_y_max,echelle3,\
           ...
           choix_sub_dark,flag_nouvelle_resolution,tnr,inSize,backend,choix_TIP,flag_TIP
    reset_FS()
    reset_general_FS()
    flag_TIP = 0
    choix_TIP.set(0)
...
def commande_noir_blanc() :
    ...
    while flag_acquisition_en_cours == True : # on attend que les acquisitions soient interrompues
        time.sleep(0.1)
    while flag_filtre_work == True : # on attend que les acquisitions soient interrompues
        time.sleep(0.05)
...
def start_cap_dark() :
    ...
    if askyesno("Cover the Lens", "Dark acquisition continue ?") :
        ...
        print("on arrete")
        flag_sub_dark = False
        labelInfoDark = Label (cadre, text = "Dark non disponible")
        labelInfoDark.place(anchor="w", x=xdark,y=ydark)
...
CBGR.place(anchor="w",x=1450, y=555)
...
echelle60.place(anchor="w", x=1530,y=555)

# Choix Parametre Atenuation Gradient Removal
labelParam61 = Label (cadre, text = "At")
labelParam61.place(anchor="e", x=1720,y=555)
...
echelle62.place(anchor="w", x=1640,y=600)
...
CBIN.place(anchor="w",x=1450, y=650)
...
CBHE2.place(anchor="w",x=1520, y=650)
...
echelle16.place(anchor="w", x=1580,y=650)
...
echelle5.place(anchor="w", x=1580,y=695)

labelParam6 = Label (cadre, text = "Max") # choix valeur histogramme strech maximum
labelParam6.place(anchor="w", x=1700,y=695)
...
echelle6.place(anchor="w", x=1720,y=695)
...
echelle12.place(anchor="w", x=1580,y=740)

labelParam13 = Label (cadre, text = "Dec") # choix valeur histogramme Signoide param 2
labelParam13.place(anchor="w", x=1700,y=740)
...
echelle13.place(anchor="w", x=1720,y=740)
...
CBCC.place(anchor="w",x=1450, y=785)

labelParam9 = Label (cadre, text = "Clip") # choix valeur contrate CLAHE
labelParam9.place(anchor="w", x=1555,y=785)
...
echelle9.place(anchor="w", x=1580,y=785)
...
labelInfoDark.place(anchor="w", x=1750,y=830)
...
CBAD.place(anchor="w",x=1685, y=830)
...
echelle10.place(anchor="w", x=1575,y=830)
