...
def satellites_tracking ():
    ...
    if flag_noir_blanc == 0 and flag_colour_camera == True :
        ...
        try :
            ...
            diff_int = np.int_(imggrey2) - np.int_(imggrey1)
            diff = np.uint8(np.clip(diff_int,0,255))
#            diff[0:50,0:750] = 0
            imggrey1 = imggrey2
        ...
        if flag_noir_blanc == 0 and flag_colour_camera == True:
            ...
            if flag_contrast_CLAHE ==1 :
                ...
                if flag_type_clahe == 2 :
                    ...
                    image_brut_grey_blur = gaussianblur_mono(image_brut_CV_grey,5)
            ...
            if flag_SAT == True :
                ...
                HSV = cv2.cvtColor(colour_base_image, cv2.COLOR_BGR2HSV).astype("float32")
                (h, s, v) = cv2.split(HSV)
                s = s * (1.0 + val_SAT/30)
                s = np.clip(s,0,255)
                HSV = cv2.merge([h,s,v])
                image_saturee = cv2.cvtColor(HSV.astype("uint8"), cv2.COLOR_HSV2BGR)
...
echelle70 = Scale (cadre, from_ = 0, to = 30, command= choix_val_SAT, orient=VERTICAL, length = 150, width = 10, resolution = 1, label="",showvalue=1,tickinterval=None,sliderlength=10)
